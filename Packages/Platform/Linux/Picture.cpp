...
//#endif
#include "../../../Dependencies/lpng1634/png.h"
#include <jpeglib.h>
...
namespace nitisa
{
	namespace platform
	{
		...
		#pragma region Helpers
			...
			void CPicture::LoadJpeg(unsigned char *data, const int size, Image &result)
			{
			...
			bool CPicture::LoadJpeg(unsigned char *data, const int size, ITexture *dest)
			{
				bool result{ false };
				struct jpeg_decompress_struct cinfo;
				struct jpeg_error_mgr jerr;
				...
				if (jpeg_read_header(&cinfo, TRUE) == JPEG_HEADER_OK)
				{
					...
					if (cinfo.output_width > 0 && cinfo.output_height > 0 && (cinfo.out_color_components == 1 || cinfo.out_color_components == 3))
					{
						...
						ITexture::IData *data{ dest->Lock(TextureFormat::RGBA, 0, true) };
						if (data)
						{
							buffer = (*cinfo.mem->alloc_sarray)((j_common_ptr)&cinfo, JPOOL_IMAGE, cinfo.output_width * cinfo.output_components, 1);
							int y{ 0 };
