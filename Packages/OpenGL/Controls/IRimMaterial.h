...
namespace nitisa
{
	...
	namespace opengl
	{
		class IRimMaterial :public virtual IMaterialControl
		{
		protected:
			IRimMaterial() = default;
			~IRimMaterial() = default;
			IRimMaterial(const IRimMaterial &other) = delete;
			IRimMaterial(IRimMaterial &&other) = delete;
			IRimMaterial &operator=(const IRimMaterial &other) = delete;
			IRimMaterial &operator=(IRimMaterial &&other) = delete;
		public:
			...
			virtual Color getRimColor() = 0; // Return rim color
			...
			virtual float getRimPower() = 0; // Return rim power
			...
			virtual bool setRimColor(const Color &value) = 0; // Set rim color
			...
			virtual bool setRimPower(const float value) = 0; // Set rim power
		};
	}
}