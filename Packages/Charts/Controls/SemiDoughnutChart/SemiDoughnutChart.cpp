...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CSemiDoughnutChart::CSemiDoughnutChart() :
			CCustomRadialChart(L"SemiDoughnutChart", true, true, false, false, false, false),
			m_fBorderWidth{ 1 },
			m_fQuality{ 0.2f },
			m_fCutout{ 0.5f },
		...
		{
			setSize(PointF{ 210, 105 });
		}

		CSemiDoughnutChart::CSemiDoughnutChart(IForm *parent) :CSemiDoughnutChart()
		{
			setForm(parent);
		}

		CSemiDoughnutChart::CSemiDoughnutChart(IControl *parent) : CSemiDoughnutChart()
		{
			setParent(parent);
		}
	#pragma endregion
	...
	#pragma region IControl setters
		bool CSemiDoughnutChart::setDPI(const Point &value)
		{
			Point old{ getDPI() };
	...
	#pragma region Getters
		float CSemiDoughnutChart::getBorderWidth() const
		{
			return m_fBorderWidth;
		}

		float CSemiDoughnutChart::getQuality() const
		{
			return m_fQuality;
		}

		float CSemiDoughnutChart::getCutout() const
		{
			return m_fCutout;
		}

		bool CSemiDoughnutChart::isRestartColors() const
		{
			return m_bRestartColors;
		}

		const ColorArray &CSemiDoughnutChart::getNormalColors() const
		{
			return m_aNormalColors;
		}

		const ColorArray &CSemiDoughnutChart::getActiveColors() const
		{
			return m_aActiveColors;
		}

		const ColorArray &CSemiDoughnutChart::getNormalBorderColors() const
		{
			return m_aNormalBorderColors;
		}

		const ColorArray &CSemiDoughnutChart::getActiveBorderColors() const
		{
	...
	#pragma region Setters
		bool CSemiDoughnutChart::setBorderWidth(const float value)
		{
		...
		bool CSemiDoughnutChart::setQuality(const float value)
		{
		...
		bool CSemiDoughnutChart::setCutout(const float value)
		{
			if (ntl::IsGreater<float>(value, 0) && ntl::IsLess<float>(value, 1) && ntl::IsNotEqual<float>(value, m_fCutout))
			{
				m_fCutout = value;
				if (m_pDataSet && m_pDataSet->getDataCount() > 0)
				{
		...
		bool CSemiDoughnutChart::setRestartColors(const bool value)
		{
		...
		bool CSemiDoughnutChart::setNormalColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiDoughnutChart::setActiveColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiDoughnutChart::setNormalBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiDoughnutChart::setActiveBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
	...
	#pragma region Helpers
		void CSemiDoughnutChart::UpdateFromStyle(IStyle *style)
		{
		...
		bool CSemiDoughnutChart::UpdateHoveredPoint(const PointF &position)
		{
			...
			if (m_pDataSet && m_pDataSet->getDataCount() > 0 && isEnabled(true))
			{
				...
				if (!client.is_zero())
				{
					...
					if (p.X >= client.Left && p.X < client.Right && p.Y >= client.Top && p.Y < client.Bottom)
					{
						p.X -= client.Left;
						p.Y -= client.Top;
						PointF center{ (client.Left + client.Right) * 0.5f, client.Bottom };
						float radius{ ntl::Min<float>(client.width(), client.height()) };
						float dr{ radius * (1 - m_fCutout) / (float)m_pDataSet->getDataCount() }, r_outer, r_inner, sum, a1, a2, a;
									...
									if (d < r_outer * r_outer && d >= r_inner * r_inner)
									{
										...
										for (int j = 0; j < m_pDataSet->getData(i)->getCount(); j++)
											if (m_pDataSet->getData(i)->getY(j) >= 0)
											{
												...
												a2 = a1 + m_pDataSet->getData(i)->getY(j) / sum * ntl::Pi<float>;
												a = Angle(p.X, p.Y, center.X, center.Y) + ntl::Pi<float>;
	...
	#pragma region Paint
		void CSemiDoughnutChart::RenderDataSet(IRenderer *renderer, const RectF &rect)
		{
			if (m_pDataSet && m_pDataSet->getDataCount() > 0)
			{
				PointF center{ (rect.Left + rect.Right) * 0.5f, rect.Bottom };
				float radius{ ntl::Min<float>(rect.width(), rect.height()) };
				float dr{ radius * (1 - m_fCutout) / (float)m_pDataSet->getDataCount() }, r_outer, r_inner;
				int color_index{ 0 };
				for (int i = 0; i < m_pDataSet->getDataCount(); i++)
					if (m_pDataSet->getData(i)->getCount() > 0)
					{
		...
		void CSemiDoughnutChart::RenderData(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() == 1)
			{
				Color color{
				...
				for (int i = 0; i < data->getCount(); i++)
				{
					...
					if (a > 0)
					{
						...
						if (color.A > 0)
						{
							...
							da = a * ntl::Pi<float> / (float)triangle_count;
							for (int j = 0; j < triangle_count; j++)
							{
								polygon[0] = PointF{ center.X - std::cos(a1 + da * (float)j) * r_inner, center.Y - std::sin(a1 + da * (float)j) * r_inner };
								polygon[1] = PointF{ center.X - std::cos(a1 + da * (float)j) * r_outer, center.Y - std::sin(a1 + da * (float)j) * r_outer };
								polygon[2] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * r_outer, center.Y - std::sin(a1 + da * float(j + 1)) * r_outer };
								polygon[3] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * r_inner, center.Y - std::sin(a1 + da * float(j + 1)) * r_inner };
		...
		void CSemiDoughnutChart::RenderBorders(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() > 1)
			{
				float sum{ 0 };
				...
				for (int i = 0; i < data->getCount(); i++)
				{
					...
					if (a > 0)
					{
						...
						if (border_color.A > 0)
						{
							...
							{
								polygon[0] = PointF{ center.X - std::cos(a1 + da * (float)j) * (r_outer - m_fBorderWidth), center.Y - std::sin(a1 + da * (float)j) * (r_outer - m_fBorderWidth) };
								polygon[1] = PointF{ center.X - std::cos(a1 + da * (float)j) * r_outer, center.Y - std::sin(a1 + da * (float)j) * r_outer };
								polygon[2] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * r_outer, center.Y - std::sin(a1 + da * float(j + 1)) * r_outer };
								polygon[3] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * (r_outer - m_fBorderWidth), center.Y - std::sin(a1 + da * float(j + 1)) * (r_outer - m_fBorderWidth) };
								renderer->DrawPolygon(polygon, border_color);

								polygon[0] = PointF{ center.X - std::cos(a1 + da * (float)j) * r_inner, center.Y - std::sin(a1 + da * (float)j) * r_inner };
								polygon[1] = PointF{ center.X - std::cos(a1 + da * (float)j) * (r_inner + m_fBorderWidth), center.Y - std::sin(a1 + da * (float)j) * (r_inner + m_fBorderWidth) };
								polygon[2] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * (r_inner + m_fBorderWidth), center.Y - std::sin(a1 + da * float(j + 1)) * (r_inner + m_fBorderWidth) };
								polygon[3] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * r_inner, center.Y - std::sin(a1 + da * float(j + 1)) * r_inner };
								renderer->DrawPolygon(polygon, border_color);
							}

							line_points.resize(2);
							line_points[0] = PointF{ center.X - std::cos(a1) * (r_inner + m_fBorderWidth), center.Y - std::sin(a1) * (r_inner + m_fBorderWidth) };
							line_points[1] = PointF{ center.X - std::cos(a1) * (r_outer - m_fBorderWidth), center.Y - std::sin(a1) * (r_outer - m_fBorderWidth) };
							...
							line_points[0] = PointF{ center.X - std::cos(a2) * (r_outer - m_fBorderWidth), center.Y - std::sin(a2) * (r_outer - m_fBorderWidth) };
							line_points[1] = PointF{ center.X - std::cos(a2) * (r_inner + m_fBorderWidth), center.Y - std::sin(a2) * (r_inner + m_fBorderWidth) };
							MakeThickPath(line_points, false, -m_fBorderWidth, false, polygon_points);
							polygon[0] = polygon_points[0].first;
							polygon[1] = polygon_points[1].first;
							polygon[2] = polygon_points[1].second;
