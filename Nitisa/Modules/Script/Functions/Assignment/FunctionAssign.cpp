...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionAssign::CFunctionAssign(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2, IFunction *modify) :
		...
		void CFunctionAssign::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionAssign::Run()
		{
			...
			if (!m_pOperand2->Run())
				return false;
			m_sResult = *m_pOperand1->Result = *m_pModify->Result = *m_pOperand2->Result;
			return true;
		}
	#pragma endregion
	}
