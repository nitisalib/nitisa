...
namespace nitisa
{
	namespace platform
	{
		namespace windows
		{
			namespace opengl45d
			{
				const Version2 MIN_OPENGL_VERSION{ 4, 5 };
				...
				const AnsiString ShaderVertex{
					#include "../../../../OpenGL/Shaders/GL45/vertex.glsl"
				};
				...
				const AnsiString ShaderFragmentMini{
					#include "../../../../OpenGL/Shaders/GL45/fragment-mini.glsl"
				};
				const AnsiString ShaderFragmentMain{
					#include "../../../../OpenGL/Shaders/GL45/fragment-main.glsl"
				};
			...
			#pragma region CTextureReleaseListener
				...
				void CRenderer::CTextureReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
						...
						for (size_t i = 0; i < m_pRenderer->m_aMasks.size(); i++)
							if (m_pRenderer->m_aMasks[i].Texture == texture)
							{
								...
								if (m_pRenderer->m_pActiveProgramBase && !m_pRenderer->m_pActiveMask)
								{
									if (m_pRenderer->m_aMasks.size() == 0)
									{
										if (!m_pRenderer->m_pActiveProgram)
										{
											m_pRenderer->m_pProgramMain->m_aSubroutines[m_pRenderer->m_pProgramMain->m_sMask.SubroutineTest] = m_pRenderer->m_pProgramMain->m_sMask.SubroutineTest_None;
											m_pRenderer->m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pRenderer->m_pProgramMain->m_aSubroutines);
										}
										else
										{
											m_pRenderer->m_pActiveProgramMini->m_aSubroutines[m_pRenderer->m_pActiveProgramMini->m_sMask.SubroutineTest] = m_pRenderer->m_pActiveProgramMini->m_sMask.SubroutineTest_None;
											m_pRenderer->m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pRenderer->m_pActiveProgramMini->m_aSubroutines);
										}
									}
									else
									{
										m_pRenderer->m_pGraphics->BindTextureUnit(3, *reinterpret_cast<const GLuint*>(&m_pRenderer->m_aMasks[m_pRenderer->m_aMasks.size() - 1].Texture->Handle));
									}
								}
					...
					auto p1{ std::find(m_pRenderer->m_aFileTextures.begin(), m_pRenderer->m_aFileTextures.end(), texture) };
					if (p1 != m_pRenderer->m_aFileTextures.end())
						m_pRenderer->m_aFileTextures.erase(p1);
					bool found{ false };
					auto p2{ std::find(m_pRenderer->m_aTextures.begin(), m_pRenderer->m_aTextures.end(), texture) };
			...
			#pragma region CTextureMultisampleReleaseListener
				...
				void CRenderer::CTextureMultisampleReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
					...
					auto p{ std::find(m_pRenderer->m_aTextureMultisamples.begin(), m_pRenderer->m_aTextureMultisamples.end(), texture) };
			...
			#pragma region CPlatformFontReleaseListener
				...
				void CRenderer::CPlatformFontReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
					...
					auto p{ std::find(m_pRenderer->m_aFonts.begin(), m_pRenderer->m_aFonts.end(), font) };
			...
			#pragma region CProgramReleaseListener
				...
				void CRenderer::CProgramReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
					...
					auto pos{ std::find(m_pRenderer->m_aPrograms.begin(), m_pRenderer->m_aPrograms.end(), program) };
			...
			#pragma region CFramebufferReleaseListener
				...
				void CRenderer::CFramebufferReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
					...
					auto pos{ std::find(m_pRenderer->m_aFramebuffers.begin(), m_pRenderer->m_aFramebuffers.end(), framebuffer) };
			...
			#pragma region CVertexArrayReleaseListener
				...
				void CRenderer::CVertexArrayReleaseListener::NotifyOnRelease(IReleasable *sender)
				{
					...
					auto pos{ std::find(m_pRenderer->m_aVertexArrays.begin(), m_pRenderer->m_aVertexArrays.end(), va) };
			...
			#pragma region Constructor & destructor
				CRenderer::CRenderer(
					...
					opengl::CBaseRenderer(double_buffered, multisample, max_version),
				...
				void CRenderer::CleanUp()
				{
					...
					// OpenGL context
					if (*reinterpret_cast<HGLRC*>(&m_hContext))
					{
						wglMakeCurrent(0, 0);
						wglDeleteContext(*reinterpret_cast<HGLRC*>(&m_hContext));
						*reinterpret_cast<HGLRC*>(&m_hContext) = 0;
					}
					// Graphics
					if (m_pGraphics)
					{
					...
					// Windows context
					if (*reinterpret_cast<HDC*>(&m_hWindowDC))
					{
						ReleaseDC(*reinterpret_cast<const HWND*>(&m_pWindow->Handle), *reinterpret_cast<HDC*>(&m_hWindowDC));
						*reinterpret_cast<HDC*>(&m_hWindowDC) = 0;
					}
					// Properties
				...
				{
					if (!(*reinterpret_cast<HDC*>(&m_hWindowDC) = GetDC(*reinterpret_cast<const HWND*>(&m_pWindow->Handle))))
					{
						m_eInitializationStatus = InitializationStatus::DeviceContext;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_GET_WINDOW_DEVICE_CONTEXT);
						return false;
					}
					// Initialize NGL
					...
					if (!ngl::wglChoosePixelFormatARB)
					{
						m_eInitializationStatus = InitializationStatus::Feature;
						ReleaseDC(*reinterpret_cast<const HWND*>(&m_pWindow->Handle), *reinterpret_cast<HDC*>(&m_hWindowDC));
						*reinterpret_cast<HDC*>(&m_hWindowDC) = 0;
						m_pGraphics->Release();
						m_pGraphics = nullptr;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_WGL_CHOOSE_PIXEL_FORMAT_ARB_UNDEFINED);
						return false;
					}
					if (!ngl::wglCreateContextAttribsARB)
					{
						m_eInitializationStatus = InitializationStatus::Feature;
						ReleaseDC(*reinterpret_cast<const HWND*>(&m_pWindow->Handle), *reinterpret_cast<HDC*>(&m_hWindowDC));
						*reinterpret_cast<HDC*>(&m_hWindowDC) = 0;
						...
						m_pGraphics = nullptr;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_WGL_CREATE_CONTENT_ATTRIBS_ARB_UNDEFINED);
						return false;
					}
					// Choose pixel format
					int pf_index;
					if (!FindPixelFormat(m_hWindowDC, pf_index, m_bDoubleBuffered, m_iMultisample))
					{
					...
					const int context_attributes[]{
						WGL_CONTEXT_MAJOR_VERSION_ARB, m_sVersion.Major,
						WGL_CONTEXT_MINOR_VERSION_ARB, m_sVersion.Minor,
						WGL_CONTEXT_FLAGS_ARB, flags,
						0 // End of attributes list
					};
					PIXELFORMATDESCRIPTOR pfd;
					ZeroMemory(&pfd, sizeof(PIXELFORMATDESCRIPTOR));
					if (!SetPixelFormat(*reinterpret_cast<HDC*>(&m_hWindowDC), pf_index, &pfd))
					{
						...
						*reinterpret_cast<HDC*>(&m_hWindowDC) = 0;
						...
						m_pGraphics = nullptr;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_SET_PIXEL_FORMAT);
						return false;
					}
					// Create OpenGL context
					if (!(*reinterpret_cast<HGLRC*>(&m_hContext) = ngl::wglCreateContextAttribsARB(*reinterpret_cast<HDC*>(&m_hWindowDC), 0, context_attributes)))
					{
						...
						*reinterpret_cast<HDC*>(&m_hWindowDC) = 0;
						...
						m_pGraphics = nullptr;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_CREATE_CONTEXT);
						return false;
					}
					// Activate OpenGL context
					if (!wglMakeCurrent(*reinterpret_cast<HDC*>(&m_hWindowDC), *reinterpret_cast<HGLRC*>(&m_hContext)))
					{
					...
					if (m_iMultisample > 0)
					{
						m_pGraphics->Enable(GL_MULTISAMPLE);
						m_pGraphics->Enable(GL_SAMPLE_SHADING);
						m_pGraphics->MinSampleShading(1);
					}
					...
					m_pGraphics->Enable(GL_PRIMITIVE_RESTART);
					...
					m_pGraphics->PrimitiveRestartIndex((unsigned int)-1);

					// Vertex array
					if (!(m_pDefaultVertexArray = opengl45::CVertexArrayImmutable::Create(m_pGraphics, DEFAULT_VERTEX_FORMAT, MaxPolygonPoints)))
					{
						CleanUp();
					...
					m_pGraphics->BindVertexArray(*reinterpret_cast<GLuint*>(&m_pDefaultVertexArray->m_hHandle));
					...
					// Framebuffer
					if (!(m_pDefaultFramebuffer = opengl45::CFramebuffer::Create(m_pGraphics, this)))
					{
						CleanUp();
				...
				bool CRenderer::CreateMainProgram()
				{
					...
					if (!vs.Compiled)
						return false;
					COpenGLShader bs{ m_pGraphics, ShaderHeader + ShaderFragmentBlock, GL_FRAGMENT_SHADER };
					if (!bs.Compiled)
						return false;
					COpenGLShader ss{ m_pGraphics, ShaderHeader + ShaderFragmentSpline, GL_FRAGMENT_SHADER };
					if (!ss.Compiled)
						return false;
					COpenGLShader su{ m_pGraphics, ShaderHeader + ShaderFragmentUtils, GL_FRAGMENT_SHADER };
					if (!su.Compiled)
						return false;
					COpenGLShader fs{ m_pGraphics, ShaderHeader + ShaderFragmentMain, GL_FRAGMENT_SHADER };
					if (!fs.Compiled)
						return false;
					COpenGLProgram *program{ new COpenGLProgram(m_pGraphics) };
					program->AddShader(vs);
					program->AddShader(bs);
					program->AddShader(ss);
					program->AddShader(su);
					program->AddShader(fs);
					if (program->Link())
					{
						if ((m_pProgramMain = opengl45::CProgramMain::Create(m_pGraphics, program)))
						{
							m_pProgramMain->RegisterReleaseListener(&m_cProgramReleaseListener);
							m_pActiveProgramBase = cast<opengl45::CProgramBase*>(m_pProgramMain);
							m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMatrices.Primitive, 1, GL_TRUE, Identity.Data);
							return true;
						}
					}
					else
			...
			#pragma region IRenderer helper methods
				void CRenderer::AddTexture(ITexture *texture)
				{
					...
					auto p{ std::find(m_aTextures.begin(), m_aTextures.end(), texture) };
				...
				void CRenderer::AddProgram(IProgram *program)
				{
					...
					auto p{ std::find(m_aPrograms.begin(), m_aPrograms.end(), program) };
				...
				void CRenderer::AddFramebuffer(IFramebuffer *framebuffer)
				{
					...
					auto p{ std::find(m_aFramebuffers.begin(), m_aFramebuffers.end(), framebuffer) };
				...
				void CRenderer::AddVertexArray(IVertexArray *vertex_array)
				{
					...
					auto p{ std::find(m_aVertexArrays.begin(), m_aVertexArrays.end(), vertex_array) };
				...
				void CRenderer::AddTextureMultisample(ITextureMultisample *texture)
				{
					...
					auto p{ std::find(m_aTextureMultisamples.begin(), m_aTextureMultisamples.end(), texture) };
			...
			#pragma region IRenderer setters
				bool CRenderer::setWindow(IWindow *value, const bool fast_switch, const bool force)
				{
					...
					{
						HDC dc{ GetDC(*reinterpret_cast<const HWND*>(&value->Handle)) };
						if (!dc)
						{
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_GET_WINDOW_DEVICE_CONTEXT);
							return false;
						}
						CWindow *w{ cast<CWindow*>(value) };
						if (!w)
						{
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_UNSUPPORTED_WINDOW_INSTANCE);
							return false;
						}
						if (w->m_iPixelFormatIndex != m_iPixelFormatIndex)
						{
							// Set pixel format
							int flags{ WGL_CONTEXT_FORWARD_COMPATIBLE_BIT_ARB };
							if (m_bUseDebugContext)
							...
							PIXELFORMATDESCRIPTOR pfd;
							ZeroMemory(&pfd, sizeof(PIXELFORMATDESCRIPTOR));
							if (!SetPixelFormat(dc, m_iPixelFormatIndex, &pfd))
							{
								ReleaseDC(*reinterpret_cast<const HWND*>(&value->Handle), dc);
								m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_SET_PIXEL_FORMAT);
								return false;
							}
							w->m_iPixelFormatIndex = m_iPixelFormatIndex;
						}
						if (!wglMakeCurrent(dc, *reinterpret_cast<HGLRC*>(&m_hContext)))
						{
							ReleaseDC(*reinterpret_cast<const HWND*>(&value->Handle), dc);
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_ACTIVATE_CONTEXT);
							return false;
						...
						if (!force)
							ReleaseDC(*reinterpret_cast<const HWND*>(&m_pWindow->Handle), *reinterpret_cast<HDC*>(&m_hWindowDC));
						*reinterpret_cast<HDC*>(&m_hWindowDC) = dc;
						m_pWindow = value;
						return true;
					}
					CleanUp();
			...
			#pragma region IRenderer active state setters
				IRenderer *CRenderer::ActivateProgram(IProgram *value)
				{
					if (value != m_pActiveProgram)
					{
						...
						if (m_pActiveProgram)
						{
							m_pActiveProgramBase = cast<opengl45::CProgramBase*>(value);
							m_pActiveProgramMini = cast<opengl45::CProgramMini*>(value);
						...
						if (m_pActiveProgramBase)
							m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMatrices.FormToGL, 1, GL_TRUE, m_sMatrixFormToGL.Data);
					...
					if (value != m_pActiveTarget)
					{
						...
						if (m_pActiveTarget)
						{
							...
							if (m_pActiveProgramBase && !m_pActiveMask)
							{
								if (!m_pActiveProgram)
								{
									if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sMask.SubroutineTest] != m_pProgramMain->m_sMask.SubroutineTest_None)
									{
										m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sMask.SubroutineTest] = m_pProgramMain->m_sMask.SubroutineTest_None;
										m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
									}
								}
								else
								{
									if (m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sMask.SubroutineTest] != m_pActiveProgramMini->m_sMask.SubroutineTest_None)
									{
										m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sMask.SubroutineTest] = m_pActiveProgramMini->m_sMask.SubroutineTest_None;
										m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
									}
								}
							}
						}
						else
						{
							...
							{
								m_pGraphics->BindTextureUnit(3, *reinterpret_cast<const GLuint*>(&m_aMasks[m_aMasks.size() - 1].Texture->Handle));
								m_pGraphics->ProgramUniform2f(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Size, m_aMasks[0].Texture->Widthf, m_aMasks[0].Texture->Heightf);
								m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Matrix, 1, GL_TRUE, m_aMasks[0].MInv.Data);
								if (!m_pActiveProgram)
								{
									if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sMask.SubroutineTest] != m_pProgramMain->m_sMask.SubroutineTest_Exists)
									{
										m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sMask.SubroutineTest] = m_pProgramMain->m_sMask.SubroutineTest_Exists;
										m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
									}
								}
								else
								{
									if (m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sMask.SubroutineTest] != m_pActiveProgramMini->m_sMask.SubroutineTest_Exists)
									{
										m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sMask.SubroutineTest] = m_pActiveProgramMini->m_sMask.SubroutineTest_Exists;
										m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
									}
								}
							}
				...
				IRenderer *CRenderer::ActivateMask(ITexture *value)
				{
					...
					if (value != m_pActiveMask)
					{
						...
						if (m_pActiveMask)
						{
							m_pGraphics->BindTextureUnit(3, *reinterpret_cast<const GLuint*>(&m_pActiveMask->Handle));
							m_pGraphics->ProgramUniform2f(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Size, m_pActiveMask->Widthf, m_pActiveMask->Heightf);
							m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Matrix, 1, GL_TRUE, Identity.Data);
							if (!m_pActiveProgram)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sMask.SubroutineTest] = m_pProgramMain->m_sMask.SubroutineTest_Exists;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
				...
				IRenderer *CRenderer::ActivateBlock(const nitisa::Block *value)
				{
					...
					if (value || m_pActiveBlock)
					{
						if (value)
						{
							...
							if (!m_pActiveProgram)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBlock.SubroutineTest] = m_pProgramMain->m_sBlock.SubroutineTest_Exists;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
							}
							else
							{
								m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBlock.SubroutineTest] = m_pActiveProgramMini->m_sBlock.SubroutineTest_Exists;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
							}
							SetBlockData(m_sActiveBlock.Rect, m_sActiveBlock.Border, m_sActiveBlock.Radius, 0);
							m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.PointLocation, (int)m_sActiveBlock.Location);
						}
						else
						{
							m_pActiveBlock = nullptr;
							if (!m_pActiveProgram)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBlock.SubroutineTest] = m_pProgramMain->m_sBlock.SubroutineTest_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
							}
							else
							{
								m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBlock.SubroutineTest] = m_pActiveProgramMini->m_sBlock.SubroutineTest_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
							}
						}
					}
				...
				IRenderer *CRenderer::ActivateBitmask(const Bitmask *value)
				{
					...
					if (value || m_pActiveBitmask)
					{
						if (value)
						{
							...
							if (!m_pActiveProgram)
							{
								m_pGraphics->ProgramUniform1ui(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sBitmask.X, m_sActiveBitmask.X);
								if (m_sActiveBitmask.is1D)
								{
									m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBitmask.SubroutineBitmask] = m_pProgramMain->m_sBitmask.SubroutineBitmask_1D;
								}
								else
								{
									if (m_sActiveBitmask.useFormCoords)
										m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBitmask.SubroutineBitmask] = m_pProgramMain->m_sBitmask.SubroutineBitmask_Form;
									else
										m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBitmask.SubroutineBitmask] = m_pProgramMain->m_sBitmask.SubroutineBitmask_2D;
									m_pGraphics->ProgramUniform1ui(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sBitmask.Y, m_sActiveBitmask.Y);
								}
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
							}
							else
							{
								m_pGraphics->ProgramUniform1ui(*reinterpret_cast<const GLuint*>(&m_pActiveProgramMini->Handle), m_pActiveProgramMini->m_sBitmask.X, m_sActiveBitmask.X);
								if (m_sActiveBitmask.is1D)
								{
									m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBitmask.SubroutineBitmask] = m_pActiveProgramMini->m_sBitmask.SubroutineBitmask_1D;
								}
								else
								{
									if (m_sActiveBitmask.useFormCoords)
										m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBitmask.SubroutineBitmask] = m_pActiveProgramMini->m_sBitmask.SubroutineBitmask_Form;
									else
										m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBitmask.SubroutineBitmask] = m_pActiveProgramMini->m_sBitmask.SubroutineBitmask_2D;
									m_pGraphics->ProgramUniform1ui(*reinterpret_cast<const GLuint*>(&m_pActiveProgramMini->Handle), m_pActiveProgramMini->m_sBitmask.Y, m_sActiveBitmask.Y);
								}
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
							}
						}
						else
						{
							...
							if (!m_pActiveProgram)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sBitmask.SubroutineBitmask] = m_pProgramMain->m_sBitmask.SubroutineBitmask_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
							}
							else
							{
								m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sBitmask.SubroutineBitmask] = m_pActiveProgramMini->m_sBitmask.SubroutineBitmask_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
							}
						}
					}
					...
					if (value || m_pActiveSplineLimits)
					{
						if (value)
						{
								...
								(ntl::IsNotZero<float>(b[0], ntl::Tolerance<float>) || ntl::IsNotZero<float>(b[1], ntl::Tolerance<float>) || ntl::IsNotZero<float>(b[2], ntl::Tolerance<float>)))
							{
								...
								m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sSpline.A, 4, spline_a);
								m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sSpline.B, 4, spline_b);
								m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sSpline.AConsts, 4, a);
								m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sSpline.BConsts, 4, b);
								if (!m_pActiveProgram)
								{
									m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sSpline.SubroutineSplineLimit] = m_sActiveSplineLimits.isY ? m_pProgramMain->m_sSpline.SubroutineSplineLimit_Y : m_pProgramMain->m_sSpline.SubroutineSplineLimit_X;
									m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
								}
								else
								{
									m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sSpline.SubroutineSplineLimit] = m_sActiveSplineLimits.isY ? m_pActiveProgramMini->m_sSpline.SubroutineSplineLimit_Y : m_pActiveProgramMini->m_sSpline.SubroutineSplineLimit_X;
									m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
								}
							}
						}
						else
						{
							...
							if (!m_pActiveProgram)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sSpline.SubroutineSplineLimit] = m_pProgramMain->m_sSpline.SubroutineSplineLimit_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMain::SUBROUTINE_COUNT, m_pProgramMain->m_aSubroutines);
							}
							else
							{
								m_pActiveProgramMini->m_aSubroutines[m_pActiveProgramMini->m_sSpline.SubroutineSplineLimit] = m_pActiveProgramMini->m_sSpline.SubroutineSplineLimit_None;
								m_pGraphics->UniformSubroutinesuiv(GL_FRAGMENT_SHADER, opengl45::CProgramMini::SUBROUTINE_COUNT, m_pActiveProgramMini->m_aSubroutines);
							}
						}
					}
				...
				IRenderer *CRenderer::ActivatePrimitiveMatrix(const Mat4f *value)
				{
					...
					if (value || m_pActivePrimitiveMatrix)
					{
						...
						m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMatrices.Primitive, 1, GL_TRUE, m_sActivePrimitiveMatrix.Data);
					}
					return this;
				}
			...
			#pragma region IRenderer platform fonts
				IPlatformFont *CRenderer::CreatePlatformFont(const String &fontname, const int height, const FontWeight weight, const bool italic, const bool underline, const bool strikeout, const bool monospace)
				{
					...
					*const_cast<FontWeight*>(&m_cSearchPlatformFont.Weight) = weight;
					...
					*const_cast<bool*>(&m_cSearchPlatformFont.Monospace) = monospace;
					auto p{ std::lower_bound(m_aFonts.begin(), m_aFonts.end(), &m_cSearchPlatformFont, CompareFonts) };
					if (p != m_aFonts.end() &&
						(*p)->FontName == fontname &&
						(*p)->Height == height &&
						(*p)->Weight == weight &&
					...
					HFONT handle{ CreateFontW(height, 0, 0, 0, w, italic, underline, strikeout, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, fontname.c_str()) };
					if (!handle)
					{
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_CREATE_FONT);
						return nullptr;
					}
					HDC dc{ CreateCompatibleDC(*reinterpret_cast<HDC*>(&m_hWindowDC)) };
					if (!dc)
					{
						DeleteObject(handle);
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_CREATE_COMPATIBLE_DEVICE_CONTEXT);
						return nullptr;
					}
					CPlatformFont *f{ new CPlatformFont(this) };
					f->RegisterReleaseListener(&m_cPlatformFontReleaseListener);
					*reinterpret_cast<HFONT*>(&f->m_hHandle) = handle;
					*reinterpret_cast<HDC*>(&f->m_hDC) = dc;
			...
			#pragma region IRenderer programs
				IProgram *CRenderer::CreateProgram(const AnsiString &vertex, const AnsiString &fragment, const Bindings &bindings)
				{
					...
					if (program->Link())
					{
						opengl::CProgram *result{ new opengl::CProgram(m_pGraphics, program) };
				...
				IProgram *CRenderer::CreateProgram(const AnsiString &fragment)
				{
					...
					if (!vs.Compiled)
						return nullptr;
					COpenGLShader fs{ m_pGraphics, ShaderHeader + ShaderFragmentMini, GL_FRAGMENT_SHADER };
					if (!fs.Compiled)
						return nullptr;
					COpenGLShader f2{ m_pGraphics, fragment, GL_FRAGMENT_SHADER };
					if (!f2.Compiled)
						return nullptr;
					COpenGLShader bs{ m_pGraphics, ShaderHeader + ShaderFragmentBlock, GL_FRAGMENT_SHADER };
					if (!bs.Compiled)
					...
					program->AddShader(bs);
					program->AddShader(su);
					program->AddShader(f2);
					program->AddShader(fs);
					if (program->Link())
					{
						IProgram *result{ opengl45::CProgramMini::Create(m_pGraphics, program) };
						if (result)
						{
							result->RegisterReleaseListener(&m_cProgramReleaseListener);
							m_aPrograms.push_back(result);
			...
			#pragma region IRenderer framebuffers
				...
				{
					opengl45::CFramebuffer *result{ opengl45::CFramebuffer::Create(m_pGraphics, this) };
					if (result)
					{
			...
			#pragma region IRenderer vertex arrays
				...
				{
					IVertexArray *result{ opengl45::CVertexArrayDynamic::Create(m_pGraphics, format) };
					if (result)
					{
				...
				IVertexArray *CRenderer::CreateVertexArray(const VertexFormat &format, const size_t vertex_count, const bool immutable)
				{
					IVertexArray *result;
					if (immutable)
						result = opengl45::CVertexArrayImmutable::Create(m_pGraphics, format, vertex_count);
					else
						result = opengl45::CVertexArrayDynamic::Create(m_pGraphics, format, vertex_count);
					if (result)
					{
						result->RegisterReleaseListener(&m_cVertexArrayReleaseListener);
						m_aVertexArrays.push_back(result);
				...
				IVertexArray *CRenderer::CreateVertexArray(const VertexFormat &format, const size_t polygon_count, const size_t polygon_size, const bool immutable)
				{
					IVertexArray *result;
					if (immutable)
						result = opengl45::CVertexArrayImmutable::Create(m_pGraphics, format, polygon_count, polygon_size);
					else
						result = opengl45::CVertexArrayDynamic::Create(m_pGraphics, format, polygon_count, polygon_size);
					if (result)
					{
						result->RegisterReleaseListener(&m_cVertexArrayReleaseListener);
						m_aVertexArrays.push_back(result);
			...
			#pragma region IRenderer textures
				ITexture *CRenderer::CreateTexture(const int width, const int height, const int level_count, const TextureFormat format, const bool precomputed)
				{
					...
					opengl45::CTexture *t{ new opengl45::CTexture(m_pGraphics, this, precomputed) };
				...
				ITexture *CRenderer::CreateTextureFromImage(const nitisa::Image &data, const int level_count, const TextureFormat format, const bool precomputed)
				{
					...
					*const_cast<bool*>(&m_cSearchTexture.Precomputed) = precomputed;
					auto p{ std::lower_bound(m_aImageTextures.begin(), m_aImageTextures.end(), m_sSearchImageTexture, CompareImageTextures) };
					...
					opengl45::CTexture *result{ new opengl45::CTexture(m_pGraphics, this, precomputed) };
					...
					if (result->m_iActualLevelCount > 1)
						m_pGraphics->GenerateTextureMipmap(*reinterpret_cast<GLuint*>(&result->m_hHandle));
				...
				ITexture *CRenderer::CreateTextureFromFile(const String &filename, const int level_count, const TextureFormat format, const bool precomputed)
				{
					...
					*const_cast<TextureFormat*>(&m_cSearchTexture.Format) = format;
					*const_cast<bool*>(&m_cSearchTexture.Precomputed) = precomputed;
					auto p{ std::lower_bound(m_aFileTextures.begin(), m_aFileTextures.end(), &m_cSearchTexture, CompareTextures) };
				...
				ITextureMultisample *CRenderer::CreateTextureMultisample(const int width, const int height, const TextureFormat format, const int samples, const bool fixed_sample_locations)
				{
					...
					opengl45::CTextureMultisample *t{ new opengl45::CTextureMultisample(m_pGraphics, this, samples, fixed_sample_locations) };
			...
			#pragma region IRenderer helpers
				...
				IRenderer *CRenderer::RestoreState()
				{
					if (m_pWindow)
					{
						...
						m_pGraphics->Disable(GL_DEPTH_TEST);
						m_pGraphics->BlendFuncSeparate(GL_ONE, GL_ONE_MINUS_SRC_ALPHA, GL_ONE_MINUS_DST_ALPHA, GL_ONE);
						m_pGraphics->PolygonMode(GL_FRONT_AND_BACK, GL_FILL);
						...
						{
							m_pGraphics->BindTextureUnit(1, *reinterpret_cast<const GLuint*>(&m_pGradientPositions->Handle));
							m_pGraphics->BindTextureUnit(2, *reinterpret_cast<const GLuint*>(&m_pGradientColors->Handle));
						}
			...
			#pragma region Masks
				bool CRenderer::PushMask(ITexture *mask, const Mat4f &matrix)
				{
					...
					if (m_aMasks.size() > 0)
					{
						...
						SetSubroutine(spMask);
						PolygonSetRectangle(first->Rectf);
						m_pGraphics->ClearColor(0, 0, 0, 0);
						m_pGraphics->Clear(GL_COLOR_BUFFER_BIT);
						m_pGraphics->ProgramUniformMatrix4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Matrix, 1, GL_TRUE, m.Data);
						m_pGraphics->BindTextureUnit(0, *reinterpret_cast<const GLuint*>(&first->Handle));
						m_pGraphics->BindTextureUnit(3, *reinterpret_cast<const GLuint*>(&mask->Handle));
						m_pGraphics->ProgramUniform2f(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sMask.Size, mask->Widthf, mask->Heightf);
			...
			#pragma region Helpers
				...
				void CRenderer::SetSubroutine(const SUBROUTINE_PATTERN pattern)
				{
					if (!m_pActiveProgram)
					{
						bool changed{ false };
						...
						{
						case spColor:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Color)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Color;
								changed = true;
							}
							break;
						case spGradient:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Gradient)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Gradient;
								changed = true;
							}
							break;
						case spTexture:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Texture)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Texture;
								changed = true;
							}
							break;
						case spTextureMultisample:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_TextureMultisample)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_TextureMultisample;
								changed = true;
							}
							break;
						case spBlock:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Block)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Block;
								changed = true;
							}
							break;
						case spText:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Text)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Text;
								changed = true;
							}
							break;
						case spBlurHGaussian:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_BlurHGaussian)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_BlurHGaussian;
								changed = true;
							}
							break;
						case spBlurVGaussian:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_BlurVGaussian)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_BlurVGaussian;
								changed = true;
							}
							break;
						case spBlurHAverage:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_BlurHAverage)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_BlurHAverage;
								changed = true;
							}
							break;
						case spBlurVAverage:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_BlurVAverage)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_BlurVAverage;
								changed = true;
							}
							break;
						case spMask:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Mask)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Mask;
								changed = true;
							}
							break;
						case spChecker:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_Checker)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_Checker;
								changed = true;
							}
							break;
						case spHSVGradient:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_HSVGradient)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_HSVGradient;
								changed = true;
							}
							break;
						case spHSVPlane:
							if (m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] != m_pProgramMain->m_sPattern.SubroutinePattern_HSVPlane)
							{
								m_pProgramMain->m_aSubroutines[m_pProgramMain->m_sPattern.SubroutinePattern] = m_pProgramMain->m_sPattern.SubroutinePattern_HSVPlane;
								changed = true;
							}
							break;
						}
				...
				void CRenderer::PolygonSetLine(const PointF &p1, const PointF &p2, const Color &c1, const Color &c2)
				{
					...
					if (m_pActiveBitmask)
					{
						...
						m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * 2, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
					}

					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * 2, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * 2, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				}
				...
				void CRenderer::PolygonSetLines(const std::vector<PointF> &points, const Color &color, const size_t offset, const size_t count)
				{
					...
					if (m_pActiveBitmask)
						m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * count, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * count, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * count, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				}
				...
				void CRenderer::PolygonSetTriangle(const PointF &p1, const PointF &p2, const PointF &p3, const Color &c1, const Color &c2, const Color &c3)
				{
					...
					if (m_pActiveBitmask)
					{
						...
						m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * 3, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
					}
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * 3, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * 3, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				}
				...
				void CRenderer::PolygonSetRectangle(const RectF &rect, const Color &c1, const Color &c2, const Color &c3, const Color &c4)
				{
					...
					if (m_pActiveBitmask)
					{
						...
						m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * 4, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
					}
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_T], 0, sizeof(Vec2f) * 4, m_pDefaultVertexArray->m_aVec2f[IDX_T].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * 4, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * 4, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				}
				...
				void CRenderer::PolygonSetRectangle(const RectF &rect, const PointF &grid_size, const Color &c1, const Color &c2)
				{
					...
					m_pGraphics->ProgramUniform4f(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sChecker.Color1, c1.R / 255.0f, c1.G / 255.0f, c1.B / 255.0f, c1.A / 255.0f);
					m_pGraphics->ProgramUniform4f(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sChecker.Color2, c2.R / 255.0f, c2.G / 255.0f, c2.B / 255.0f, c2.A / 255.0f);
				}
				...
				void CRenderer::SetGradient(nitisa::Gradient &g, const bool precomputed)
				{
					...
					d2->Release();
					d1->Release();
					m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.GradientType, (int)g.Type);
					m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.GradientSize, point_count);
					m_pGraphics->BindTextureUnit(1, *reinterpret_cast<const GLuint*>(&m_pGradientPositions->Handle));
					m_pGraphics->BindTextureUnit(2, *reinterpret_cast<const GLuint*>(&m_pGradientColors->Handle));
				}
				...
				void CRenderer::PolygonSetBlock(const RectF &rect, const RectF &block, const RectF &border, const RectF &radius, const BlockColors &colors)
				{
					...
					if (m_pActiveProgramBase)
					{
						...
						m_pGraphics->ProgramUniform4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.Colors, 6, gl_colors);
					}
				}
				...
				void CRenderer::PolygonSetPolygon(const std::vector<PointF> &points, const Color &color)
				{
					...
					if (m_pActiveBitmask)
						m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * count, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_T], 0, sizeof(Vec2f) * count, m_pDefaultVertexArray->m_aVec2f[IDX_T].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * count, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
					m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * count, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				}
				...
				{
					m_pGraphics->ProgramUniform4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.Rect[index], 1, block.Data);
					m_pGraphics->ProgramUniform4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.Border[index], 1, border.Data);
					m_pGraphics->ProgramUniform4fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.Radius[index], 1, radius.Data);
					...
					m_pGraphics->ProgramUniform2fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.CornerCenter[index], 4, vec2four);
					...
					m_pGraphics->ProgramUniform4iv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.HasInner[index], 1, has_inner);
					...
					m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.InnerScale[index], 4, vec4);
					...
					m_pGraphics->ProgramUniform1fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.InnerRadius[index], 4, vec4);
					...
					m_pGraphics->ProgramUniform2fv(*reinterpret_cast<const GLuint*>(&m_pActiveProgramBase->Handle), m_pActiveProgramBase->m_sBlock.OuterRCenter[index], 4, vec2four);
				}
			...
			#pragma region Draw start/stop
				bool CRenderer::DrawBegin(Rect &viewport, IWindow *window)
				{
					...
					if (window != m_pWindow && window)
					{
						HDC dc{ GetDC(*reinterpret_cast<const HWND*>(&window->Handle)) };
						if (!dc)
						{
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_GET_WINDOW_DEVICE_CONTEXT);
							return false;
						...
						if (w->m_iPixelFormatIndex != m_iPixelFormatIndex)
						{
							...
							if (!SetPixelFormat(dc, m_iPixelFormatIndex, &pfd))
							{
								ReleaseDC(*reinterpret_cast<const HWND*>(&window->Handle), dc);
								m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_SET_PIXEL_FORMAT);
								return false;
							}
							w->m_iPixelFormatIndex = m_iPixelFormatIndex;
				...
				bool CRenderer::Present()
				{
					...
					if (m_bDoubleBuffered)
						SwapBuffers(*reinterpret_cast<HDC*>(&m_hWindowDC));
					else
						m_pGraphics->Flush();
					return true;
				}
			...
			#pragma region IRenderer draw HSV plane
				IRenderer *CRenderer::DrawHSVPlane(const RectF &rect, const float hue)
				{
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						SetSubroutine(spHSVPlane);
						m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sHSVPlane.Hue, hue);
						m_pGraphics->DrawArrays(GL_TRIANGLE_FAN, 0, 4);
					}
					return this;
				}
			...
			#pragma region IRenderer draw image
				IRenderer *CRenderer::DrawImage(IBaseTexture *image, const float transparency)
				{
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (t)
						{
							SetSubroutine(spTexture);
							m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.Transparency, transparency);
							m_pGraphics->BindTextureUnit(0, *reinterpret_cast<const GLuint*>(&image->Handle));
						}
						else
						{
							ITextureMultisample *m{ cast<ITextureMultisample*>(image) };
							SetSubroutine(spTextureMultisample);
							m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.Transparency, transparency);
							m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.SampleCount, m->Samples);
							m_pGraphics->BindTextureUnit(4, *reinterpret_cast<const GLuint*>(&image->Handle));
						}
						m_pGraphics->DrawArrays(GL_TRIANGLE_FAN, 0, 4);
					}
					return this;
				...
				IRenderer *CRenderer::DrawText(const String &text, IPlatformFont *pf, const float distance, const Color &color)
				{
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (length > 0 && pf->getTexture())
						{
							...
							SetSubroutine(spText);
							m_pGraphics->BindTextureUnit(0, *reinterpret_cast<const GLuint*>(&pf->getTexture()->Handle));
							...
							for (size_t i = 0; i < length; i++)
							{
								...
								{
									m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_T], 0, sizeof(Vec2f) * index, m_pDefaultVertexArray->m_aVec2f[IDX_T].data());
									m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec2fIDs[IDX_B], 0, sizeof(Vec2f) * index, m_pDefaultVertexArray->m_aVec2f[IDX_B].data());
									m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec3fIDs[IDX_P], 0, sizeof(Vec3f) * index, m_pDefaultVertexArray->m_aVec3f[IDX_P].data());
									m_pGraphics->NamedBufferSubData(m_pDefaultVertexArray->m_aVec4fIDs[IDX_C], 0, sizeof(Vec4f) * index, m_pDefaultVertexArray->m_aVec4f[IDX_C].data());
				...
				IRenderer *CRenderer::BlurImage(ITexture *image, const int radius, const BlurType type)
				{
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (temp)
						{
							...
							SUBROUTINE_PATTERN s1, s2;
							switch (type)
							{
							case BlurType::Gaussian:
								s1 = spBlurHGaussian;
								s2 = spBlurVGaussian;
								m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurK, 1.19682684f / radius);
								m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurKExp, -4.5f / (radius * radius));
								break;
							default:
								s1 = spBlurHAverage;
								s2 = spBlurVAverage;
							}
							m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurRadius, radius);
							m_pGraphics->ProgramUniform2f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurDisp, 1 / image->Rectf.Right, 1 / image->Rectf.Bottom);
							// First pass
							ActivateTarget(temp);
							m_pGraphics->ClearColor(0, 0, 0, 0);
							m_pGraphics->Clear(GL_COLOR_BUFFER_BIT);
							...
							m_pGraphics->Clear(GL_COLOR_BUFFER_BIT);
							m_pGraphics->BindTextureUnit(0, *reinterpret_cast<const GLuint*>(&temp->Handle));
							SetSubroutine(s2);
							m_pGraphics->DrawArrays(GL_TRIANGLE_FAN, 0, 4);
							// Finish
							temp->Release();
				...
				IRenderer *CRenderer::BlurImage(ITexture *source, const int radius, const BlurType type, ITexture *target)
				{
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (temp)
						{
							...
							m_pGraphics->ProgramUniform1i(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurRadius, radius);
							m_pGraphics->ProgramUniform2f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurDisp, 1 / target->Rectf.Right, 1 / target->Rectf.Bottom);
							// First pass
							ActivateTarget(temp);
							m_pGraphics->ClearColor(0, 0, 0, 0);
							m_pGraphics->Clear(GL_COLOR_BUFFER_BIT);
							PolygonSetRectangle(target->Rectf);
							m_pGraphics->BindTextureUnit(0, *reinterpret_cast<const GLuint*>(&source->Handle));
							SetSubroutine(s1);
							m_pGraphics->DrawArrays(GL_TRIANGLE_FAN, 0, 4);
							// Second pass
