	...
	namespace platform
	{
		namespace windows
		{
			namespace opengl31d
			{
				...
				const AnsiString ShaderHeaderMS{
					"#version 150\n"
				};
				...
				const AnsiString ShaderFragmentMain{
					#include "../../../../OpenGL/Shaders/GL31-GLES30/fragment-main.glsl"
				};
				const AnsiString ShaderFragmentMainMS{
			...
			#pragma region Constructor & destructor
				...
				bool CRenderer::Initialize()
				{
					...
					// Vertex array
					if (!(m_pDefaultVertexArray = opengl31::CVertexArrayImmutable::Create(m_pGraphics, DEFAULT_VERTEX_FORMAT, MaxPolygonPoints)))
					{
						CleanUp();
					...
					// Framebuffer
					if (!(m_pDefaultFramebuffer = opengl31::CFramebuffer::Create(m_pGraphics, this)))
					{
						CleanUp();
				...
				{
					AnsiString header{ (m_sVersion >= Version2{ 3, 2 }) ? ShaderHeaderMS : ShaderHeader };
					COpenGLShader vs{ m_pGraphics, header + ShaderVertex, GL_VERTEX_SHADER };
					if (!vs.Compiled)
						return false;
					COpenGLShader bs{ m_pGraphics, header + ShaderFragmentBlock, GL_FRAGMENT_SHADER };
					if (!bs.Compiled)
						return false;
					COpenGLShader ss{ m_pGraphics, header + ShaderFragmentSpline, GL_FRAGMENT_SHADER };
					if (!ss.Compiled)
						return false;
					COpenGLShader su{ m_pGraphics, header + ShaderFragmentUtils, GL_FRAGMENT_SHADER };
					if (!su.Compiled)
						return false;
					COpenGLShader fs{ m_pGraphics, header + ((m_sVersion >= Version2{ 3, 2 }) ? ShaderFragmentMainMS : ShaderFragmentMain), GL_FRAGMENT_SHADER };
					if (!fs.Compiled)
						return false;
					...
					if (program->Link())
					{
						if ((m_pProgramMain = opengl31::CProgramMain::Create(m_pGraphics, program)))
						{
							m_pProgramMain->RegisterReleaseListener(&m_cProgramReleaseListener);
							m_pActiveProgramBase = cast<opengl31::CProgramBase*>(m_pProgramMain);
			...
			#pragma region IRenderer active state setters
				IRenderer *CRenderer::ActivateProgram(IProgram *value)
				{
					if (value != m_pActiveProgram)
					{
						...
						if (m_pActiveProgram)
						{
							m_pActiveProgramBase = cast<opengl31::CProgramBase*>(value);
							m_pActiveProgramMini = cast<opengl31::CProgramMini*>(value);
			...
			#pragma region IRenderer programs
				...
				IProgram *CRenderer::CreateProgram(const AnsiString &fragment)
				{
					...
					if (!vs.Compiled)
						return nullptr;
					COpenGLShader fs{ m_pGraphics, header + ShaderFragmentMini, GL_FRAGMENT_SHADER };
					if (!fs.Compiled)
						return nullptr;
					...
					if (program->Link())
					{
						IProgram *result{ opengl31::CProgramMini::Create(m_pGraphics, program) };
						if (result)
						{
							result->RegisterReleaseListener(&m_cProgramReleaseListener);
							m_aPrograms.push_back(result);
			...
			#pragma region IRenderer framebuffers
				...
				{
					opengl31::CFramebuffer *result{ opengl31::CFramebuffer::Create(m_pGraphics, this) };
					if (result)
					{
			...
			#pragma region IRenderer vertex arrays
				...
				{
					IVertexArray *result{ opengl31::CVertexArrayDynamic::Create(m_pGraphics, format) };
					if (result)
					{
				...
				IVertexArray *CRenderer::CreateVertexArray(const VertexFormat &format, const size_t vertex_count, const bool immutable)
				{
					IVertexArray *result;
					if (immutable)
						result = opengl31::CVertexArrayImmutable::Create(m_pGraphics, format, vertex_count);
					else
						result = opengl31::CVertexArrayDynamic::Create(m_pGraphics, format, vertex_count);
					if (result)
					{
						result->RegisterReleaseListener(&m_cVertexArrayReleaseListener);
						m_aVertexArrays.push_back(result);
				...
				IVertexArray *CRenderer::CreateVertexArray(const VertexFormat &format, const size_t polygon_count, const size_t polygon_size, const bool immutable)
				{
					IVertexArray *result;
					if (immutable)
						result = opengl31::CVertexArrayImmutable::Create(m_pGraphics, format, polygon_count, polygon_size);
					else
						result = opengl31::CVertexArrayDynamic::Create(m_pGraphics, format, polygon_count, polygon_size);
					if (result)
					{
						result->RegisterReleaseListener(&m_cVertexArrayReleaseListener);
						m_aVertexArrays.push_back(result);
			...
			#pragma region IRenderer textures
				ITexture *CRenderer::CreateTexture(const int width, const int height, const int level_count, const TextureFormat format, const bool precomputed)
				{
					...
					opengl31::CTexture *t{ new opengl31::CTexture(m_pGraphics, this, precomputed) };
				...
				ITexture *CRenderer::CreateTextureFromImage(const nitisa::Image &data, const int level_count, const TextureFormat format, const bool precomputed)
				{
					...
					*const_cast<bool*>(&m_cSearchTexture.Precomputed) = precomputed;
					...
					opengl31::CTexture *result{ new opengl31::CTexture(m_pGraphics, this, precomputed) };
				...
				ITextureMultisample *CRenderer::CreateTextureMultisample(const int width, const int height, const TextureFormat format, const int samples, const bool fixed_sample_locations)
				{
					...
					{
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_TEXTURE_MULTISAMPLE_NOT_SUPPORTED);
						return nullptr;
					}
					opengl31::CTextureMultisample *t{ new opengl31::CTextureMultisample(m_pGraphics, this, samples, fixed_sample_locations) };
