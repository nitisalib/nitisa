	...
	namespace platform
	{
		namespace windows
		{
			namespace opengl31
			{
				...
				const GLint Wraps[(int)TextureWrap::RepeatMirrored + 1]{ GL_CLAMP_TO_BORDER, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_REPEAT, GL_MIRRORED_REPEAT };
				...
				const GLuint PixelFormats[(int)TextureFormat::DepthStencil + 1]{ GL_RED, GL_RED, GL_RGB, GL_RGB, GL_RGBA, GL_RGBA, GL_DEPTH_COMPONENT, GL_STENCIL_INDEX, GL_DEPTH_STENCIL };
				const GLuint PixelTypes[(int)TextureFormat::DepthStencil + 1]{ GL_UNSIGNED_BYTE, GL_FLOAT, GL_UNSIGNED_BYTE, GL_FLOAT, GL_UNSIGNED_BYTE, GL_FLOAT, GL_FLOAT, GL_UNSIGNED_BYTE, GL_DEPTH24_STENCIL8 };
			...
			#pragma region CDataAlpha
					...
					m_aData{ nullptr }
				{
					...
					GLint w, h;
					m_pTexture->m_pGraphics->GetTexLevelParameteriv(GL_TEXTURE_2D, m_iLevel, GL_TEXTURE_WIDTH, &w);
					m_pTexture->m_pGraphics->GetTexLevelParameteriv(GL_TEXTURE_2D, m_iLevel, GL_TEXTURE_HEIGHT, &h);
					if (w > 0 && h > 0)
					{
						m_iWidth = w;
						m_iHeight = h;
						...
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RED, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
				...
				void CTexture::CDataAlpha::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->PixelStorei(GL_UNPACK_ALIGNMENT, 1);
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RED, GL_UNSIGNED_BYTE, reinterpret_cast<const void*>(m_aData));
			...
			#pragma region CDataAlphaFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RED, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataAlphaFloat::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RED, GL_FLOAT, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataRGB
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RGB, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataRGB::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RGB, GL_UNSIGNED_BYTE, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataRGBFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RGB, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataRGBFloat::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RGB, GL_FLOAT, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataRGBA
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RGBA, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataRGBA::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RGBA, GL_UNSIGNED_BYTE, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataRGBAFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_RGBA, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataRGBAFloat::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_RGBA, GL_FLOAT, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataDepth
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_iHeight = h;
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_DEPTH_COMPONENT, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
				...
				void CTexture::CDataDepth::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							m_pTexture->m_pGraphics->ActiveTexture(GL_TEXTURE0);
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_DEPTH_COMPONENT, GL_FLOAT, reinterpret_cast<const void*>(m_aData));
						}
						delete[] m_aData;
					}
				}
			...
			#pragma region CDataStencil
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_aData = new DATA[m_iWidth * m_iHeight];
						m_pTexture->m_pGraphics->PixelStorei(GL_PACK_ALIGNMENT, 1);
						m_pTexture->m_pGraphics->GetTexImage(GL_TEXTURE_2D, m_iLevel, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
						m_pTexture->m_pGraphics->PixelStorei(GL_PACK_ALIGNMENT, 4);
					}
				}
				...
				void CTexture::CDataStencil::BeforeRelease()
				{
					if (m_aData)
					{
						if (!m_bReadOnly)
						{
							...
							m_pTexture->m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_pTexture->m_hHandle));
							m_pTexture->m_pGraphics->PixelStorei(GL_UNPACK_ALIGNMENT, 1);
							m_pTexture->m_pGraphics->TexSubImage2D(GL_TEXTURE_2D, m_iLevel, 0, 0, m_iWidth, m_iHeight, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, reinterpret_cast<const void*>(m_aData));
							m_pTexture->m_pGraphics->PixelStorei(GL_UNPACK_ALIGNMENT, 4);
						}
						delete[] m_aData;
					}
			...
			#pragma region ITexture setters
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eDepthStencilMode && m_pRenderer->Version >= Version2{ 4, 3 })
					{
						m_eDepthStencilMode = value;
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_DEPTH_STENCIL_TEXTURE_MODE, DepthStencilModes[(int)m_eDepthStencilMode]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setBaseLevel(const int value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value >= 0 && value != m_iBaseLevel)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, (GLint)m_iBaseLevel);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setMinFilter(const TextureMinFilter value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eMinFilter)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, MinFilters[(int)m_eMinFilter]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setMagFilter(const TextureMagFilter value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eMagFilter)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, MagFilters[(int)m_eMagFilter]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setMinLOD(const float value, const float relative_tolerance)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && ntl::NotEquals<float>(value, m_fMinLOD, relative_tolerance))
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_LOD, m_fMinLOD);
						if (m_fMinLOD >= m_fMaxLOD)
						{
							m_fMaxLOD = m_fMinLOD + 1;
							m_pGraphics->TexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_LOD, m_fMaxLOD);
						}
						return true;
					}
					return false;
				...
				bool CTexture::setWrapS(const TextureWrap value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eWrapS)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, Wraps[(int)m_eWrapS]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setWrapT(const TextureWrap value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eWrapT)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, Wraps[(int)m_eWrapT]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setWrapR(const TextureWrap value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eWrapR)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, Wraps[(int)m_eWrapR]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setBorderColor(const Color &value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_sBorderColor)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, data);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setCompareMode(const TextureCompareMode value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eCompareMode)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, CompareModes[(int)m_eCompareMode]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setCompareFunction(const TextureCompareFunction value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eCompareFunction)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC, CompareFuncs[(int)m_eCompareFunction]);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setLODBias(const float value, const float relative_tolerance)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && ntl::NotEquals<float>(value, m_fLODBias, relative_tolerance))
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameterf(GL_TEXTURE_2D, GL_TEXTURE_LOD_BIAS, m_fLODBias);
						return true;
					}
					return false;
				}
				...
				bool CTexture::setMaxLevel(const int value)
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value > 0 && value != m_iMaxLevel)
					{
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, (GLint)m_iMaxLevel);
						return true;
					}
					return false;
				}
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eSwizzleR && m_pRenderer->Version >= Version2{ 3, 3 })
					{
						m_eSwizzleR = value;
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_R, Swizzles[(int)m_eSwizzleR]);
						return true;
					}
					return false;
				}
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eSwizzleG && m_pRenderer->Version >= Version2{ 3, 3 })
					{
						m_eSwizzleG = value;
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, Swizzles[(int)m_eSwizzleG]);
						return true;
					}
					return false;
				}
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eSwizzleB && m_pRenderer->Version >= Version2{ 3, 3 })
					{
						m_eSwizzleB = value;
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, Swizzles[(int)m_eSwizzleB]);
						return true;
					}
					return false;
				}
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && value != m_eSwizzleA && m_pRenderer->Version >= Version2{ 3, 3 })
					{
						m_eSwizzleA = value;
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, Swizzles[(int)m_eSwizzleA]);
						return true;
					}
					return false;
				}
				...
				{
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0 && (value != m_eSwizzleR || value != m_eSwizzleG || value != m_eSwizzleB || value != m_eSwizzleA) && m_pRenderer->Version >= Version2{ 3, 3 })
					{
						m_eSwizzleR = value;
						m_eSwizzleG = value;
						m_eSwizzleB = value;
						...
						m_pGraphics->ActiveTexture(GL_TEXTURE0);
						m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_RGBA, Swizzles[(int)m_eSwizzleR]);
						return true;
					}
					return false;
				}
			...
			#pragma region Helpers
				...
				{
					if (m_eDepthStencilMode != TextureDepthStencilMode::DepthComponent && m_pRenderer->Version >= Version2{ 4, 3 })
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_DEPTH_STENCIL_TEXTURE_MODE, DepthStencilModes[(int)m_eDepthStencilMode]);
					if (m_iBaseLevel != 0)
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, (GLint)m_iBaseLevel);
					...
					if (m_iMaxLevel != 1000)
						m_pGraphics->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, (GLint)m_iMaxLevel);
					if (m_pRenderer->Version >= Version2{ 3, 3 })
					{
				...
				void CTexture::UpdateTexture()
				{
					m_pGraphics->ActiveTexture(GL_TEXTURE0);
					m_pGraphics->BindTexture(GL_TEXTURE_2D, *reinterpret_cast<GLuint*>(&m_hHandle));
					m_iMaxLevel = m_iActualLevelCount - 1;
					UpdateParameters();
					float w{ m_sRectf.Right }, h{ m_sRectf.Bottom };
					for (int i = 0; i < m_iActualLevelCount; i++, w /= 2, h /= 2)
						m_pGraphics->TexImage2D(GL_TEXTURE_2D, i, Formats[(int)m_eFormat], ntl::Max<int>(1, ntl::Round<int, float>(w)), ntl::Max<int>(1, ntl::Round<int, float>(h)), 0,
							PixelFormats[(int)m_eFormat], PixelTypes[(int)m_eFormat], nullptr);
					m_bCreated = true;
