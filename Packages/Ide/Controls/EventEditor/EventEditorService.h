...
namespace nitisa
{
	...
	namespace ide
	{
		...
		class CEventEditorService :public CControlService
		{
		private:
			...
			bool FocusHandler(const int index);
			bool ScrollToHandler();
			void UpdateHandlerItems(IEvent *event); // Add to handler all available compatible event names
			void ProcessKeyUp();
			void ProcessKeyDown();
			...
			void ProcessKeyEnd();
		public:
			CEventEditorService(CEventEditor *editor);
