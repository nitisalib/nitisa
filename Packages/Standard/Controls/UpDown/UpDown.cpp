...
namespace nitisa
{
	...
	#pragma region CGradientListener
		CUpDown::CGradientListener::CGradientListener(CUpDown *control) :
			m_pControl{ control }
		{

		}

		void CUpDown::CGradientListener::NotifyOnChange()
		{
	...
	#pragma region Constructor & destructor
		CUpDown::CUpDown():
			CControl(L"UpDown", true, true, false, false, false, false),
			...
			m_eDownElement{ Element::None }
		{
			...
			setService(new CUpDownService(this), true);
		...
		CUpDown::CUpDown(IForm *parent) : CUpDown()
		{
			setForm(parent);
		}

		CUpDown::CUpDown(IControl *parent) : CUpDown()
		{
			setParent(parent);
		}
	#pragma endregion
	...
	#pragma region Interface getters
		...
		int CUpDown::getValue()
		{
			int v;
			if (m_pAssignedTo && TryStringToInt(m_pAssignedTo->getText(), v))
			{
				v = ntl::Clamp<int>(v, m_iMin, m_iMax);
				if (v != m_iValue)
				{
					m_iValue = v;
					m_pAssignedTo->setText(ToString(m_iValue));
				}
			}
			return m_iValue;
		}
	...
	#pragma region Getters
		CUpDown::State CUpDown::getState()
		{
			if (!isEnabled(false))
		...
		float CUpDown::getTimerInterval()
		{
			return m_fTimerInterval;
		}

		float CUpDown::getDistance(const State state)
		{
			return m_aDistance[(int)state];
		}

		RectF CUpDown::getUpArrowOuterBorderWidth(const State state)
		{
			return m_aUpArrowOuterBorderWidth[(int)state];
		}

		BorderColor CUpDown::getUpArrowOuterBorderColor(const State state)
		{
			return m_aUpArrowOuterBorderColor[(int)state];
		}

		RectF CUpDown::getUpArrowMiddleBorderWidth(const State state)
		{
			return m_aUpArrowMiddleBorderWidth[(int)state];
		}

		BorderColor CUpDown::getUpArrowMiddleBorderColor(const State state)
		{
			return m_aUpArrowMiddleBorderColor[(int)state];
		}

		RectF CUpDown::getUpArrowInnerBorderWidth(const State state)
		{
			return m_aUpArrowInnerBorderWidth[(int)state];
		}

		BorderColor CUpDown::getUpArrowInnerBorderColor(const State state)
		{
			return m_aUpArrowInnerBorderColor[(int)state];
		}

		Color CUpDown::getUpArrowBackgroundColor(const State state)
		{
			return m_aUpArrowBackgroundColor[(int)state];
		}

		Gradient *CUpDown::getUpArrowBackgroundGradient(const State state)
		{
			return &m_aUpArrowBackgroundGradient[(int)state];
		}

		RectF CUpDown::getUpArrowPadding(const State state)
		{
			return m_aUpArrowPadding[(int)state];
		}

		Color CUpDown::getUpArrowArrowColor(const State state)
		{
			return m_aUpArrowArrowColor[(int)state];
		}

		RectF CUpDown::getDownArrowOuterBorderWidth(const State state)
		{
			return m_aDownArrowOuterBorderWidth[(int)state];
		}

		BorderColor CUpDown::getDownArrowOuterBorderColor(const State state)
		{
			return m_aDownArrowOuterBorderColor[(int)state];
		}

		RectF CUpDown::getDownArrowMiddleBorderWidth(const State state)
		{
			return m_aDownArrowMiddleBorderWidth[(int)state];
		}

		BorderColor CUpDown::getDownArrowMiddleBorderColor(const State state)
		{
			return m_aDownArrowMiddleBorderColor[(int)state];
		}

		RectF CUpDown::getDownArrowInnerBorderWidth(const State state)
		{
			return m_aDownArrowInnerBorderWidth[(int)state];
		}

		BorderColor CUpDown::getDownArrowInnerBorderColor(const State state)
		{
			return m_aDownArrowInnerBorderColor[(int)state];
		}

		Color CUpDown::getDownArrowBackgroundColor(const State state)
		{
			return m_aDownArrowBackgroundColor[(int)state];
		}

		Gradient *CUpDown::getDownArrowBackgroundGradient(const State state)
		{
			return &m_aDownArrowBackgroundGradient[(int)state];
		}

		RectF CUpDown::getDownArrowPadding(const State state)
		{
			return m_aDownArrowPadding[(int)state];
		}

		Color CUpDown::getDownArrowArrowColor(const State state)
		{
			return m_aDownArrowArrowColor[(int)state];
		}

		bool CUpDown::isChangeByDrag() const
		{
			return m_bChangeByDrag;
		}

		float CUpDown::getDragShift() const
		{
			return m_fDragShift;
		}

		float CUpDown::getDragScale() const
		{
			return m_fDragScale;
		}

		float CUpDown::getDragBase() const
		{
			return m_fDragBase;
		}

		float CUpDown::getDragBaseShift() const
		{
			return m_fDragBaseShift;
		}

		float CUpDown::getDragBaseScale() const
		{
	...
	#pragma region Interface setters
		bool CUpDown::setDPI(const Point &value)
		{
			Point old{ getDPI() };
		...
		bool CUpDown::setAlign(const Align value)
		{
			return false;
		}

		bool CUpDown::setMin(const int value)
		{
			if (value != m_iMin)
			{
				CLockRepaint lock{ this };
		...
		bool CUpDown::setMax(const int value)
		{
			if (value != m_iMax)
			{
				CLockRepaint lock{ this };
		...
		bool CUpDown::setValue(const int value)
		{
		...
		bool CUpDown::setStep(const int value)
		{
			if (value > 0 && value != m_iStep)
			{
				m_iStep = value;
				return true;
			}
		...
		bool CUpDown::setAssignedTo(IEdit *value)
		{
	...
	#pragma region Setters
		bool CUpDown::setTimerInterval(const float value)
		{
		...
		bool CUpDown::setDistance(const State state, const float value)
		{
		...
		bool CUpDown::setUpArrowOuterBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setUpArrowOuterBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setUpArrowMiddleBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setUpArrowMiddleBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setUpArrowInnerBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setUpArrowInnerBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setUpArrowBackgroundColor(const State state, const Color &value)
		{
		...
		bool CUpDown::setUpArrowPadding(const State state, const RectF &value)
		{
		...
		bool CUpDown::setUpArrowArrowColor(const State state, const Color &value)
		{
		...
		bool CUpDown::setDownArrowOuterBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setDownArrowOuterBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setDownArrowMiddleBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setDownArrowMiddleBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setDownArrowInnerBorderWidth(const State state, const RectF &value)
		{
		...
		bool CUpDown::setDownArrowInnerBorderColor(const State state, const BorderColor &value)
		{
		...
		bool CUpDown::setDownArrowBackgroundColor(const State state, const Color &value)
		{
		...
		bool CUpDown::setDownArrowPadding(const State state, const RectF &value)
		{
		...
		bool CUpDown::setDownArrowArrowColor(const State state, const Color &value)
		{
		...
		bool CUpDown::setChangeByDrag(const bool value)
		{
		...
		bool CUpDown::setDragShift(const float value, const float tolerance)
		{
		...
		bool CUpDown::setDragScale(const float value, const float tolerance)
		{
		...
		bool CUpDown::setDragBase(const float value, const float tolerance)
		{
		...
		bool CUpDown::setDragBaseShift(const float value, const float tolerance)
		{
		...
		bool CUpDown::setDragBaseScale(const float value, const float tolerance)
		{
	...
	#pragma region Helpers
		void CUpDown::Refresh(const bool refresh_children)
		{
			if (m_pCanvas)
				m_pCanvas->setValid(false);
		}
		...
		bool CUpDown::Up()
		{
			return setValue(getValue() + m_iStep);
		}
		...
		bool CUpDown::UpdateHoveredElement()
		{
			if (getForm())
		...
		bool CUpDown::UpdateHoveredElement(const PointF &position)
		{
		...
		CUpDown::State CUpDown::getUpArrowState()
		{
			if (!isEnabled(false))
				return State::Disabled;
		...
		CUpDown::State CUpDown::getDownArrowState()
		{
			if (!isEnabled(false))
				return State::Disabled;
			if (m_eDownElement == Element::DownArrow)
