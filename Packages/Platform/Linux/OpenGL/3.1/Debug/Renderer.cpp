						...
						for (size_t i = 0; i < m_pRenderer->m_aMasks.size(); i++)
							if (m_pRenderer->m_aMasks[i].Texture == texture)
							{
								...
								if (m_pRenderer->m_pActiveProgramBase && !m_pRenderer->m_pActiveMask)
								{
									if (m_pRenderer->m_aMasks.size() == 0)
									{
										m_pRenderer->m_pGraphics->Uniform1i(m_pRenderer->m_pActiveProgramBase->m_sMask.SubroutineTest, m_pRenderer->m_pActiveProgramBase->m_sMask.NoMask);
									}
									else
									{
			...
			#pragma region IRenderer active state setters
					...
					if (value != m_pActiveTarget)
					{
						...
						if (m_pActiveTarget)
						{
							...
							if (m_pActiveProgramBase && !m_pActiveMask)
								m_pGraphics->Uniform1i(m_pActiveProgramBase->m_sMask.SubroutineTest, m_pActiveProgramBase->m_sMask.NoMask);
						}
						else
						{
				...
				IRenderer *CRenderer::ActivateBlock(const nitisa::Block *value)
				{
					...
					if (value || m_pActiveBlock)
					{
						...
						{
							m_pActiveBlock = nullptr;
							m_pGraphics->Uniform1i(m_pActiveProgramBase->m_sBlock.SubroutineTest, m_pActiveProgramBase->m_sBlock.NoBlock);
						}
					}
					return this;
				}
				...
				IRenderer *CRenderer::ActivateSplineLimits(const CubicBezierSplineLimits *value)
				{
					...
					if (value || m_pActiveSplineLimits)
					{
						...
						{
							m_pActiveSplineLimits = nullptr;
							m_pGraphics->Uniform1i(m_pActiveProgramBase->m_sSpline.SubroutineSplineLimit, m_pActiveProgramBase->m_sSpline.NoLimits);
						}
					}
					return this;
				}
					...
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (temp)
						{
							...
							case BlurType::Gaussian:
								...
								m_pGraphics->Uniform1f(m_pProgramMain->m_sPattern.BlurK, 1.19682684f / (float)radius);
								m_pGraphics->Uniform1f(m_pProgramMain->m_sPattern.BlurKExp, -4.5f / float(radius * radius));
								break;
							default:
