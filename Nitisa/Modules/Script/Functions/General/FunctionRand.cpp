...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionRand::CFunctionRand(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2) :
			CFunction(L"rand", error_listener),
		...
		void CFunctionRand::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionRand::Run()
		{
			...
			if (!m_pOperand2->Run())
				return false;
			std::uniform_int_distribution<int64> dist{ int64(*m_pOperand1->Result), int64(*m_pOperand2->Result) };
			m_sResult = dist(m_cEngine);
			return true;
		}
	#pragma endregion
	}
