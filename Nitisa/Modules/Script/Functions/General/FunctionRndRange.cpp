// This file is a part of Nitisa framework
// Copyright © 2022 Nitisa. All rights reserved.
// Author: Dimitry Lysenko
// Site: http://nitisa.com
// Download: http://nitisa.com/downloads
// Documentation: http://nitisa.com/documentation
// License: http://nitisa.com/site/license

#include "stdafx.h"

namespace nitisa
{
	namespace script
	{
	#pragma region Constructor & destructor
		CFunctionRndRange::CFunctionRndRange(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2) :
			CFunction(L"rnd", error_listener),
			m_pOperand1{ operand1 },
			m_pOperand2{ operand2 }
		{
			m_pResult = &m_sResult;
			auto now{ std::chrono::system_clock::now() };
			std::chrono::milliseconds duration{ std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()) };
			m_cEngine.seed((unsigned int)duration.count());
		}

		void CFunctionRndRange::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
				m_pOperand2->Release();
		}
	#pragma endregion

	#pragma region IFunction methods
		bool CFunctionRndRange::Run()
		{
			if (!m_pOperand1)
			{
				if (m_pErrorListener)
				{
					m_pErrorListener->NotifyOnError(
						__FUNCTION__,
						__LINE__,
						ERROR_FUNCTION_MISSING_OPERAND_1,
						StringKeyValueArray{ StringKeyValue{ L"{name}", m_sName } }
					);
				}
				return false;
			}
			if (!m_pOperand2)
			{
				if (m_pErrorListener)
				{
					m_pErrorListener->NotifyOnError(
						__FUNCTION__,
						__LINE__,
						ERROR_FUNCTION_MISSING_OPERAND_2,
						StringKeyValueArray{ StringKeyValue{ L"{name}", m_sName } }
					);
				}
				return false;
			}
			if (!m_pOperand1->Run())
				return false;
			if (!m_pOperand2->Run())
				return false;
			std::uniform_real_distribution<double> dist{ double(*m_pOperand1->Result), double(*m_pOperand2->Result) };
			m_sResult = dist(m_cEngine);
			return true;
		}
	#pragma endregion
	}
}