			...
			#pragma region ITextureMultisample setters
				bool CTextureMultisample::setFormat(const TextureFormat value)
				{
					if (value != m_eFormat)
					{
						...
						{
							m_pGraphics->CreateTextures(GL_TEXTURE_2D_MULTISAMPLE, 1, reinterpret_cast<GLuint*>(&m_hHandle));
							if (*reinterpret_cast<GLuint*>(&m_hHandle) == 0)
								return false;
							m_pGraphics->TextureStorage2DMultisample(*reinterpret_cast<GLuint*>(&m_hHandle), m_iSamples, Formats[(int)m_eFormat], m_sRect.Right, m_sRect.Bottom, m_bFixedSampleLocations);
							UpdateParameters();
							m_bCreated = true;
