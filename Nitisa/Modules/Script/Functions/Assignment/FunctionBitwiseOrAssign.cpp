...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionBitwiseOrAssign::CFunctionBitwiseOrAssign(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2, IFunction *modify) :
		...
		void CFunctionBitwiseOrAssign::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionBitwiseOrAssign::Run()
		{
			if (!m_pOperand1)
			{
				if (m_pErrorListener)
