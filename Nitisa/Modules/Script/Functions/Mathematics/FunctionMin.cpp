...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionMin::CFunctionMin(IErrorListener *error_listener, const Arguments &arguments) :
			CFunction(L"min", error_listener),
			m_aArguments{ arguments }
		{
			m_pResult = &m_sResult;
		}

		void CFunctionMin::BeforeRelease()
		{
		...
		bool CFunctionMin::Run()
		{
			...
			if (has_string)
			{
				...
				for (size_t i = 1; i < m_aArguments.size(); i++)
					v = ntl::Min<String>(v, String(*m_aArguments[i]->Result));
				m_sResult = v;
			}
			else if (has_float)
			{
				...
				for (size_t i = 1; i < m_aArguments.size(); i++)
					v = ntl::Min<double>(v, double(*m_aArguments[i]->Result));
				m_sResult = v;
			}
			else
			{
				...
				for (size_t i = 1; i < m_aArguments.size(); i++)
					v = ntl::Min<int64>(v, int64(*m_aArguments[i]->Result));
				m_sResult = v;
			}
			return true;
		}
