...
namespace nitisa
{
	namespace platform
	{
		namespace android
		{
			...
			#else
				const GLint Wraps[(int)TextureWrap::RepeatMirrored + 1]{ GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_REPEAT, GL_MIRRORED_REPEAT };
			#endif
			...
			#pragma region CDataAlpha
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						CStoreFramebuffer framebuffer{ m_pTexture->m_pRenderer };
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->PixelStorei(GL_PACK_ALIGNMENT, 1);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RED, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
			...
			#pragma region CDataAlphaFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RED, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataRGB
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataRGBFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RGB, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataRGBA
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataRGBAFloat
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_RGBA, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataDepth
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pFramebuffer->setColorAttachment(m_pTexture, 0, level);
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_DEPTH_COMPONENT, GL_FLOAT, reinterpret_cast<void*>(m_aData));
					}
				}
			...
			#pragma region CDataStencil
					...
					m_aData{ nullptr }
				{
					...
					if (w > 0 && h > 0)
					{
						...
						m_pTexture->m_pRenderer->ActivateFramebuffer(m_pTexture->m_pFramebuffer);
						m_pTexture->m_pGraphics->PixelStorei(GL_PACK_ALIGNMENT, 1);
						m_pTexture->m_pGraphics->ReadPixels(0, 0, w, h, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(m_aData));
						m_pTexture->m_pGraphics->PixelStorei(GL_PACK_ALIGNMENT, 4);
					}
				}
