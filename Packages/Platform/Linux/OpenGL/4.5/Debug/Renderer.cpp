		...
		namespace linux
		{
			...
			#pragma region Constructor & destructor
				...
				void CRenderer::CleanUp()
				{
					...
					// OpenGL context
					if (*reinterpret_cast<GLXContext*>(&m_hContext))
					{
						glXMakeCurrent(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), 0, 0);
						glXDestroyContext(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<GLXContext*>(&m_hContext));
						*reinterpret_cast<GLXContext*>(&m_hContext) = 0;
					}
					// Graphics
					if (m_pGraphics)
					{
				...
				{
					// Initialize NGL
					ngl::PlatformData pd{ *reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<::Window*>(&m_pApplication->m_hClipboardWindow) };
					...
					if (!ngl::glXCreateContextAttribsARB)
					{
						m_eInitializationStatus = InitializationStatus::Feature;
						m_pGraphics->Release();
						m_pGraphics = nullptr;
						m_pWindow = nullptr;
						m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_GLX_CREATE_CONTENT_ATTRIBS_ARB_UNDEFINED);
						return false;
					}
					// Choose pixel format
					GLXFBConfig fb_config;
					if (!FindPixelFormat(*reinterpret_cast<PlatformHandle*>(&fb_config), m_bDoubleBuffered, m_iMultisample))
					{
					...
					const int context_attributes[]{
						GLX_CONTEXT_MAJOR_VERSION_ARB, m_sVersion.Major,
						GLX_CONTEXT_MINOR_VERSION_ARB, m_sVersion.Minor,
						GLX_CONTEXT_FLAGS_ARB, flags,
						0 // End of attributes list
					};
					if (!(*reinterpret_cast<GLXContext*>(&m_hContext) = ngl::glXCreateContextAttribsARB(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), fb_config, 0, True, context_attributes)))
					{
					...
					if (!glXMakeCurrent(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<const ::Window*>(&m_pWindow->Handle), *reinterpret_cast<GLXContext*>(&m_hContext)))
					{
			...
			#pragma region IRenderer setters
				bool CRenderer::setWindow(IWindow *value, const bool fast_switch, const bool force)
				{
					...
					{
						if (!glXMakeCurrent(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<const ::Window*>(&value->Handle), *reinterpret_cast<GLXContext*>(&m_hContext)))
						{
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_ACTIVATE_CONTEXT);
							return false;
						}
			...
			#pragma region Helpers
				void CRenderer::ActivateViewportMatrix()
				{
					m_sMatrixFormToGL =
						...
						ntl::Mat4Scale<float>(2.0f / (float)m_sViewport.width(), 2.0f / (float)m_sViewport.height(), 1) * // Transform coordinates to range 0..2
			...
			#pragma region Draw start/stop
				bool CRenderer::DrawBegin(Rect &viewport, IWindow *window)
				{
					...
					{
						if (!glXMakeCurrent(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<const ::Window*>(&window->Handle), *reinterpret_cast<GLXContext*>(&m_hContext)))
						{
							m_pListener->NotifyOnError(__FUNCTION__, __LINE__, ERROR_RENDERER_ACTIVATE_CONTEXT);
							return false;
						}
				...
				bool CRenderer::Present()
				{
					...
					if (m_bDoubleBuffered)
						glXSwapBuffers(*reinterpret_cast<Display**>(&m_pApplication->m_hHandle), *reinterpret_cast<const ::Window*>(&m_pWindow->Handle));
					else
						m_pGraphics->Flush();
					return true;
				}
					...
					if (CheckDrawAllowed(__FUNCTION__, __LINE__))
					{
						...
						if (temp)
						{
							...
							case BlurType::Gaussian:
								s1 = spBlurHGaussian;
								s2 = spBlurVGaussian;
								m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurK, 1.19682684f / (float)radius);
								m_pGraphics->ProgramUniform1f(*reinterpret_cast<const GLuint*>(&m_pProgramMain->Handle), m_pProgramMain->m_sPattern.BlurKExp, -4.5f / float(radius * radius));
								break;
							default:
								s1 = spBlurHAverage;
								s2 = spBlurVAverage;
