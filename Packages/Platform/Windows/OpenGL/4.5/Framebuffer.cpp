		...
		namespace windows
		{
			...
			#pragma region Constructor & destructor
					...
					m_pRenderer{ renderer }
				{
					...
					m_pGraphics->CreateFramebuffers(1, reinterpret_cast<GLuint*>(&m_hHandle));
					if (*reinterpret_cast<GLuint*>(&m_hHandle) > 0)
						m_pGraphics->NamedFramebufferDrawBuffer(*reinterpret_cast<GLuint*>(&m_hHandle), GL_NONE);
				}
					...
					if (value != m_pDepthAttachment || l != m_iDepthAttachmentLevel || force)
					{
						if (value != m_pDepthAttachment || force)
						{
							if (m_pDepthAttachment && !value)
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_DEPTH_ATTACHMENT, 0, 0);
							m_pDepthAttachment = value;
							if (m_pDepthAttachment)
							{
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_DEPTH_ATTACHMENT, *reinterpret_cast<const GLuint*>(&m_pDepthAttachment->Handle), l);
					...
					if (value != m_pStencilAttachment || l != m_iStencilAttachmentLevel || force)
					{
						if (value != m_pStencilAttachment || force)
						{
							if (m_pStencilAttachment && !value)
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_STENCIL_ATTACHMENT, 0, 0);
							m_pStencilAttachment = value;
							if (m_pStencilAttachment)
							{
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_STENCIL_ATTACHMENT, *reinterpret_cast<const GLuint*>(&m_pStencilAttachment->Handle), l);
					...
					if (value != m_pDepthAttachment || value != m_pStencilAttachment || l != m_iDepthAttachmentLevel || l != m_iStencilAttachmentLevel || force)
					{
						if (value != m_pDepthAttachment || value != m_pStencilAttachment || force)
						{
							if ((m_pDepthAttachment || m_pStencilAttachment) && !value)
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_DEPTH_STENCIL_ATTACHMENT, 0, 0);
							m_pDepthAttachment = value;
							m_pStencilAttachment = value;
							if (m_pDepthAttachment)
							{
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_DEPTH_STENCIL_ATTACHMENT, *reinterpret_cast<const GLuint*>(&m_pDepthAttachment->Handle), l);
								m_iDepthAttachmentLevel = l;
								m_iStencilAttachmentLevel = l;
							}
							else
					...
					if (index <= (int)m_aColorAttachments.size() || value != m_aColorAttachments[index].Texture || l != m_aColorAttachments[index].Level || force)
					{
						...
						if (value != m_aColorAttachments[index].Texture || force)
						{
							if (m_aColorAttachments[index].Texture && !value)
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_COLOR_ATTACHMENT0 + index, 0, 0);
							m_aColorAttachments[index].Texture = value;
							if (m_aColorAttachments[index].Texture)
							{
								m_pGraphics->NamedFramebufferTexture(*reinterpret_cast<GLuint*>(&m_hHandle), GL_COLOR_ATTACHMENT0 + index, *reinterpret_cast<const GLuint*>(&m_aColorAttachments[index].Texture->Handle), l);
							...
							if (count > 0)
							{
								...
								m_pGraphics->NamedFramebufferDrawBuffers(*reinterpret_cast<GLuint*>(&m_hHandle), (GLsizei)count, draw_buffers);
								delete[] draw_buffers;
							}
							else
								m_pGraphics->NamedFramebufferDrawBuffer(*reinterpret_cast<GLuint*>(&m_hHandle), GL_NONE);
