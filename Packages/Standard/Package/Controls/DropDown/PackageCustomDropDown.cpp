...
namespace nitisa
{
	namespace standard
	{
		CPackageCustomDropDown::CPackageCustomDropDown(IPackage *package, const String &ns) : CPackageControl(package, ns)
		{
			AddHeaderFile(L"Standard/Controls/DropDown/CustomDropDown.h");
		}

		void CPackageCustomDropDown::Save(Variant &dest, IPropertyList *properties, IEventList *events)
		{
			CPackageControl::Save(dest, properties, events);
			CCustomDropDown *control{ cast<CCustomDropDown*>(properties->getParent()) };
		...
		void CPackageCustomDropDown::Load(const Variant &src, IPropertyList *properties, IEventList *events)
		{
			CCustomDropDown *control{ cast<CCustomDropDown*>(properties->getParent()) };
		...
		void CPackageCustomDropDown::Export(std::wofstream &f, const String &shift, const String &owner, IPropertyList *properties, IEventList *events, const String &form, const String &dialog_box)
		{
			CCustomDropDown *control{ cast<CCustomDropDown*>(properties->getParent()) };
			if (Application->Editor && control)
				for (int i = 0; i < control->getItemCount(); i++)
