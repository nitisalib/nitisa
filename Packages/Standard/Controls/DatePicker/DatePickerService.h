...
namespace nitisa
{
	...
	namespace standard
	{
		...
		class CDatePickerService :public CControlService
		{
		private:
			...
			void ProcessKeyUp();
			void ProcessKeyDown();
			void ProcessYearsDigit(const int digit);
			void ProcessMonthsDigit(const int digit);
			void ProcessDaysDigit(const int digit);
			PointF FormToCalendar(const PointF &pos);
			bool InCalendar(const PointF &pos);
		public:
			...
			CDatePickerService(CDatePicker *control);
		};
	}
}