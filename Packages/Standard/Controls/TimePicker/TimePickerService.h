...
namespace nitisa
{
	namespace standard
	{
		...
		class CTimePickerService :public CControlService
		{
		private:
			...
			void ProcessKeyUp();
			void ProcessKeyDown();
			void ProcessHoursDigit(const int digit);
			void ProcessMinutesDigit(const int digit);
			void ProcessSecondsDigit(const int digit);
		public:
			...
			CTimePickerService(CTimePicker *control);
		};
	}
}