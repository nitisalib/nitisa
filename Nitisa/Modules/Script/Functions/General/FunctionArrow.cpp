...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionArrow::CFunctionArrow(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2) :
		...
		void CFunctionArrow::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionArrow::Run()
		{
			...
			if (!cast<CFunctionIdentifier*>(m_pOperand2))
			{
				if (m_pErrorListener)
				{
					m_pErrorListener->NotifyOnError(
