...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionPrefixDecrement::CFunctionPrefixDecrement(IErrorListener *error_listener, IFunction *operand, IFunction *modify) :
		...
		void CFunctionPrefixDecrement::BeforeRelease()
		{
			if (m_pOperand)
				m_pOperand->Release();
		}
	...
	#pragma region IFunction methods
		bool CFunctionPrefixDecrement::Run()
		{
			if (!m_pOperand)
			{
				if (m_pErrorListener)
