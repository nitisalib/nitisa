...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CViewV3::CViewV3() :
			CControl(L"ViewV3", true, true, true, false, false, false),
			m_bResizable{ true },
			m_fSeparator1{ 0.3333f },
			m_fSeparator2{ 0.6667f },
		...
		{
			setService(new CViewV3Service(this), true);
			setSize(PointF{ 640, 480 });
		}

		CViewV3::CViewV3(IForm *parent) :
			CViewV3()
		{
			setForm(parent);
		}

		CViewV3::CViewV3(IControl *parent) :
			CViewV3()
		{
			setParent(parent);
		}
	...
	#pragma region IViewControl methods
		bool CViewV3::ActivateView(const int index)
		{
			if (index < 0)
	...
	#pragma region IViewV3 getters
		bool CViewV3::isResizable()
		{
			return m_bResizable;
		}

		float CViewV3::getSeparator1()
		{
			return m_fSeparator1;
		}

		float CViewV3::getSeparator2()
		{
			return m_fSeparator2;
		}
	#pragma endregion

	#pragma region IViewV3 setters
		bool CViewV3::setResizable(const bool value)
		{
		...
		bool CViewV3::setSeparator1(const float value)
		{
			float v{ ntl::Clamp<float>(value, 0, m_fSeparator2 - m_fSeparatorWidth / ntl::Max<float>(1, getHeight())) };
			if (ntl::IsNotEqual<float>(v, m_fSeparator1, 0.0001f))
			{
				m_fSeparator1 = v;
				AlignControls();
				Repaint(false);
		...
		bool CViewV3::setSeparator2(const float value)
		{
			float v{ ntl::Clamp<float>(value, m_fSeparator1 + m_fSeparatorWidth / ntl::Max<float>(1, getHeight()), 1) };
			if (ntl::IsNotEqual<float>(v, m_fSeparator2, 0.0001f))
			{
				m_fSeparator2 = v;
				AlignControls();
				Repaint(false);
	...
	#pragma region Getters
		CViewV3::View CViewV3::getActiveView()
		{
			return m_eActiveView;
		}

		float CViewV3::getBorderWidth()
		{
			return m_fBorderWidth;
		}

		float CViewV3::getSeparatorWidth()
		{
			return m_fSeparatorWidth;
		}

		Color CViewV3::getBorderColor(const State state)
		{
			return m_aBorderColor[(int)state];
		}

		Color CViewV3::getSeparatorColor(const State state)
		{
	...
	#pragma region Setters
		bool CViewV3::setActiveView(const View value)
		{
		...
		bool CViewV3::setBorderWidth(const float value)
		{
		...
		bool CViewV3::setSeparatorWidth(const float value)
		{
			float v{ ntl::Clamp<float>(value, 0, 16) };
		...
		bool CViewV3::setBorderColor(const State state, const Color &value)
		{
		...
		bool CViewV3::setSeparatorColor(const State state, const Color &value)
		{
	...
	#pragma region Helpers
		void CViewV3::UpdateFromStyle(IStyle *style)
		{
		...
		RectF CViewV3::CalculateSeparator1Rect()
		{
			if (ntl::IsZero<float>(m_fSeparatorWidth) || getHeight() <= m_fSeparatorWidth * 2 || ntl::IsZero<float>(getWidth()))
				return RectF{ 0, 0, 0, 0 };
			RectF result{ 0, std::round(getHeight() * m_fSeparator1 - m_fSeparatorWidth * 0.5f), getWidth(), 0 };
			result.Bottom = result.Top + m_fSeparatorWidth;
			return result;
		}

		RectF CViewV3::CalculateSeparator2Rect()
		{
			if (ntl::IsZero<float>(m_fSeparatorWidth) || getHeight() <= m_fSeparatorWidth * 2 || ntl::IsZero<float>(getWidth()))
				return RectF{ 0, 0, 0, 0 };
			RectF result{ 0, std::round(getHeight() * m_fSeparator2 - m_fSeparatorWidth * 0.5f), getWidth(), 0 };
			result.Bottom = result.Top + m_fSeparatorWidth;
			return result;
		}

		RectF CViewV3::CalculateTopViewRect()
		{
			if (ntl::IsZero<float>(m_fBorderWidth) || ntl::IsZero<float>(getHeight()))
				return RectF{ 0, 0, 0, 0 };
			RectF result{ 0, 0, getWidth(), std::round(getHeight() * m_fSeparator1 - m_fSeparatorWidth * 0.5f) };
			if (result.height() <= m_fBorderWidth * 2 || result.width() <= m_fBorderWidth * 2)
				return RectF{ 0, 0, 0, 0 };
			return result;
		}

		RectF CViewV3::CalculateMiddleViewRect()
		{
			if (ntl::IsZero<float>(m_fBorderWidth) || ntl::IsZero<float>(getHeight()))
				return RectF{ 0, 0, 0, 0 };
			RectF result{ 0, std::round(getHeight() * m_fSeparator1 - m_fSeparatorWidth * 0.5f) + m_fSeparatorWidth, getWidth(), std::round(getHeight() * m_fSeparator2 - m_fSeparatorWidth * 0.5f) };
			if (result.height() <= m_fBorderWidth * 2 || result.width() <= m_fBorderWidth * 2)
				return RectF{ 0, 0, 0, 0 };
			return result;
		}

		RectF CViewV3::CalculateBottomViewRect()
		{
			if (ntl::IsZero<float>(m_fBorderWidth) || ntl::IsZero<float>(getHeight()))
				return RectF{ 0, 0, 0, 0 };
			RectF result{ 0, std::round(getHeight() * m_fSeparator2 - m_fSeparatorWidth * 0.5f) + m_fSeparatorWidth, getWidth(), getHeight() };
			if (result.height() <= m_fBorderWidth * 2 || result.width() <= m_fBorderWidth * 2)
				return RectF{ 0, 0, 0, 0 };
			return result;
		}

		RectF CViewV3::CalculateTopClientRect()
		{
			RectF result{ 0, 0, getWidth(), std::round(getHeight() * m_fSeparator1 - m_fSeparatorWidth * 0.5f) };
			result.validate();
			return result;
		...
		RectF CViewV3::CalculateMiddleClientRect()
		{
			RectF result{ 0, std::round(getHeight() * m_fSeparator1 - m_fSeparatorWidth * 0.5f) + m_fSeparatorWidth, getWidth(), std::round(getHeight() * m_fSeparator2 - m_fSeparatorWidth * 0.5f) };
			result.validate();
			return result;
		...
		RectF CViewV3::CalculateBottomClientRect()
		{
			RectF result{ 0, std::round(getHeight() * m_fSeparator2 - m_fSeparatorWidth * 0.5f) + m_fSeparatorWidth, getWidth(), getHeight() };
			result.validate();
			return result;
	...
	#pragma region Render
		void CViewV3::Render()
		{
			...
			// Separator1
			RectF rect{ CalculateSeparator1Rect() * getRect() };
			if (!rect.is_zero())
			{
				if (m_eDownElement == Element::Separator1)
				{
			...
			// Separator2
			rect = CalculateSeparator2Rect() * getRect();
			if (!rect.is_zero())
			{
				if (m_eDownElement == Element::Separator2)
				{
					if (m_aSeparatorColor[(int)State::Active].A > 0)
						renderer->DrawRectangle(rect, m_aSeparatorColor[(int)State::Active]);
				}
