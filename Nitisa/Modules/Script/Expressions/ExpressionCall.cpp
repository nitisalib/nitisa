...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CExpressionCall::CExpressionCall(const size_t index, const Operator *op, IExpression *name, IExpression *arguments) :
			CExpression(index, ExpressionType::Call, false),
			m_pOp{ op },
			m_pName{ name },
		...
		void CExpressionCall::BeforeRelease()
		{
	...
	#pragma region IExpression methods
		String CExpressionCall::toString()
		{
	...
	#pragma region Getters
		IExpression **CExpressionCall::getInnerName()
		{
			if (!m_pName)
				return nullptr;
			switch (m_pName->Type)
