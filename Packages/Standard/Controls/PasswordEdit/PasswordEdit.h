...
namespace nitisa
{
	...
	namespace standard
	{
		class CPasswordEditService;

		class CPasswordEdit :public virtual IPasswordEdit, public CControl
		{
			friend CPasswordEditService;
		private:
			class CPasswordEditTextInput :public CBuiltInTextInput
			{
			private:
				CPasswordEdit * m_pEdit;
			public:
				CPasswordEditTextInput(CPasswordEdit *edit);

				IControl *getControl() override;
				State getState() override;
			};

			class CPasswordEditListener :public virtual IBuiltInTextInputListener
			{
			private:
				CPasswordEdit * m_pEdit;
			public:
				CPasswordEditListener(CPasswordEdit *edit);
			...
			{
			private:
				CPasswordEdit *m_pControl;
			public:
				void NotifyOnChange() override;

				CGradientListener(CPasswordEdit *control);
			};
		private:
			...
			wchar_t m_cReplaceSymbol;

			bool m_bIgnoreGradient;
			bool m_bIgnoreTextInput;
			ITexture *m_pCanvas;
			CPasswordEditListener m_cListener;

			IBuiltInTextInput *getTextInput();
			void UpdateFromStyle();
			void AdjustTextInputSize();
			String getDisguisedText(const String &txt);
			RectF getRenderRect(const CBuiltInTextInput::State state);
			void Render();
		public:
			void(*OnChange)(IControl *sender); // Event called when password changed

			// IControl methods
			void Refresh(const bool refresh_children) override;

			CPasswordEdit();
			CPasswordEdit(IControl *parent);
			CPasswordEdit(IForm *parent);
			~CPasswordEdit() override;
			...
			// IPasswordEdit getters
			String getPassword() override;
			wchar_t getReplaceSymbol() override;
			// ITextInput getters
			...
			// IPasswordEdit setters
			bool setPassword(const String &value) override;
			bool setReplaceSymbol(const wchar_t value) override;
			// ITextInput setters
