	...
	namespace platform
	{
		namespace windows
		{
			...
			#pragma region IRenderer helpers
				...
				{
					if (*reinterpret_cast<HGLRC*>(&m_hContext) && wglGetCurrentContext() != *reinterpret_cast<HGLRC*>(&m_hContext))
						wglMakeCurrent(*reinterpret_cast<HDC*>(&m_hWindowDC), *reinterpret_cast<HGLRC*>(&m_hContext));
					return this;
				}
			#pragma endregion
			...
			#pragma region Helpers
				...
				bool CBaseRenderer::FindPixelFormat(PlatformHandle dc, int &pf_index, const bool double_buffered, int &multisample)
				{
					while (multisample > 0)
					{
						const int pixel_attributes[]{
							...
							WGL_SAMPLES_ARB, multisample,        //Number of samples
							0 // End of attributes list
						};
						UINT num;
						pf_index = -1;
						if (ngl::wglChoosePixelFormatARB(*reinterpret_cast<HDC*>(&dc), pixel_attributes, nullptr, 1, &pf_index, &num) && pf_index >= 0)
							return true;
						multisample--;
					}
					...
					UINT num;
					pf_index = -1;
					return ngl::wglChoosePixelFormatARB(*reinterpret_cast<HDC*>(&dc), pixel_attributes, nullptr, 1, &pf_index, &num) && pf_index >= 0;
				}
			#pragma endregion
			}
		}
