		...
		namespace android
		{
			...
			#pragma region Constructor & destructor
				CBaseRenderer::CBaseRenderer(const bool double_buffered, const int multisample, const Version2 &max_version) :
					...
					m_pGraphics{ nullptr }
				{
					if (m_pApplication->m_pRenderer)
						throw "Multiple renderers are not allowed";
					m_pWindow = nullptr;
					...
					m_pApplication->m_pRenderer = this;
				}

				void CBaseRenderer::BeforeRelease()
				{
					CleanUp();
					m_pApplication->m_pRenderer = nullptr;
				}
			#pragma endregion
			...
			#pragma region IRenderer helpers
				...
				{
					if (*reinterpret_cast<EGLContext*>(&m_hContext) && eglGetCurrentContext() != *reinterpret_cast<EGLContext*>(&m_hContext))
						eglMakeCurrent(*reinterpret_cast<EGLDisplay*>(&m_pApplication->m_hHandle), *reinterpret_cast<EGLSurface*>(&m_hSurface), *reinterpret_cast<EGLSurface*>(&m_hSurface), *reinterpret_cast<EGLContext*>(&m_hContext));
					return this;
				}
			#pragma endregion
			...
			#pragma region Helpers
				...
				bool CBaseRenderer::FindPixelFormat(PlatformHandle &config, int &multisample)
				{
					while (multisample > 0)
					{
						const EGLint attribs[] = {
							...
							EGL_SAMPLES, multisample,
							EGL_NONE
						};
						EGLint numConfigs;
						if (eglChooseConfig(*reinterpret_cast<EGLDisplay*>(&m_pApplication->m_hHandle), attribs, reinterpret_cast<EGLConfig*>(&config), 1, &numConfigs) == EGL_TRUE)
							return true;
						multisample--;
					}
					const EGLint attribs[] = {
