...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CDoughnutChart::CDoughnutChart() :
			CCustomRadialChart(L"DoughnutChart", true, true, false, false, false, false),
			m_fBorderWidth{ 1 },
			m_fQuality{ 0.2f },
			m_fCutout{ 0.5f },
		...
		CDoughnutChart::CDoughnutChart(IForm *parent) :CDoughnutChart()
		{
			setForm(parent);
		}

		CDoughnutChart::CDoughnutChart(IControl *parent) : CDoughnutChart()
		{
			setParent(parent);
		}
	#pragma endregion
	...
	#pragma region IControl setters
		bool CDoughnutChart::setDPI(const Point &value)
		{
			Point old{ getDPI() };
	...
	#pragma region Getters
		float CDoughnutChart::getBorderWidth() const
		{
			return m_fBorderWidth;
		}

		float CDoughnutChart::getQuality() const
		{
			return m_fQuality;
		}

		float CDoughnutChart::getCutout() const
		{
			return m_fCutout;
		}

		bool CDoughnutChart::isRestartColors() const
		{
			return m_bRestartColors;
		}

		const ColorArray &CDoughnutChart::getNormalColors() const
		{
			return m_aNormalColors;
		}

		const ColorArray &CDoughnutChart::getActiveColors() const
		{
			return m_aActiveColors;
		}

		const ColorArray &CDoughnutChart::getNormalBorderColors() const
		{
			return m_aNormalBorderColors;
		}

		const ColorArray &CDoughnutChart::getActiveBorderColors() const
		{
	...
	#pragma region Setters
		bool CDoughnutChart::setBorderWidth(const float value)
		{
		...
		bool CDoughnutChart::setQuality(const float value)
		{
		...
		bool CDoughnutChart::setCutout(const float value)
		{
		...
		bool CDoughnutChart::setRestartColors(const bool value)
		{
		...
		bool CDoughnutChart::setNormalColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CDoughnutChart::setActiveColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CDoughnutChart::setNormalBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CDoughnutChart::setActiveBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
	...
	#pragma region Helpers
		void CDoughnutChart::UpdateFromStyle(IStyle *style)
		{
		...
		bool CDoughnutChart::UpdateHoveredPoint(const PointF &position)
		{
	...
	#pragma region Paint
		void CDoughnutChart::RenderDataSet(IRenderer *renderer, const RectF &rect)
		{
			if (m_pDataSet && m_pDataSet->getDataCount() > 0)
			{
		...
		void CDoughnutChart::RenderData(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() == 1)
			{
				Color color{
		...
		void CDoughnutChart::RenderBorders(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() > 1)
			{
				float sum{ 0 };
