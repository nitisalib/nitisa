...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CSemiPieChart::CSemiPieChart() :
			CCustomRadialChart(L"SemiPieChart", true, true, false, false, false, false),
			m_fBorderWidth{ 1 },
			m_fQuality{ 0.2f },
		...
		CSemiPieChart::CSemiPieChart(IForm *parent) :CSemiPieChart()
		{
			setForm(parent);
		}

		CSemiPieChart::CSemiPieChart(IControl *parent) : CSemiPieChart()
		{
			setParent(parent);
		}
	#pragma endregion
	...
	#pragma region IControl setters
		bool CSemiPieChart::setDPI(const Point &value)
		{
			Point old{ getDPI() };
	...
	#pragma region Getters
		float CSemiPieChart::getBorderWidth() const
		{
			return m_fBorderWidth;
		}

		float CSemiPieChart::getQuality() const
		{
			return m_fQuality;
		}

		bool CSemiPieChart::isRestartColors() const
		{
			return m_bRestartColors;
		}

		const ColorArray &CSemiPieChart::getNormalColors() const
		{
			return m_aNormalColors;
		}

		const ColorArray &CSemiPieChart::getActiveColors() const
		{
			return m_aActiveColors;
		}

		const ColorArray &CSemiPieChart::getNormalBorderColors() const
		{
			return m_aNormalBorderColors;
		}

		const ColorArray &CSemiPieChart::getActiveBorderColors() const
		{
	...
	#pragma region Setters
		bool CSemiPieChart::setBorderWidth(const float value)
		{
		...
		bool CSemiPieChart::setQuality(const float value)
		{
		...
		bool CSemiPieChart::setRestartColors(const bool value)
		{
		...
		bool CSemiPieChart::setNormalColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiPieChart::setActiveColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiPieChart::setNormalBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSemiPieChart::setActiveBorderColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
	...
	#pragma region Helpers
		void CSemiPieChart::UpdateFromStyle(IStyle *style)
		{
		...
		bool CSemiPieChart::UpdateHoveredPoint(const PointF &position)
		{
	...
	#pragma region Paint
		void CSemiPieChart::RenderDataSet(IRenderer *renderer, const RectF &rect)
		{
			if (m_pDataSet && m_pDataSet->getDataCount() > 0)
			{
		...
		void CSemiPieChart::RenderData(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() == 1)
			{
				Color color{
					...
					if (a > 0)
					{
						...
						if (color.A > 0)
						{
							...
							if (is_central)
							{
								...
								polygon[0] = center;
								polygon[1] = PointF{ center.X - std::cos(a1) * r_outer, center.Y - std::sin(a1) * r_outer };
								for (int j = 0; j < triangle_count; j++)
									polygon[j + 2] = PointF{ center.X - std::cos(a1 + da * float(j + 1)) * r_outer, center.Y - std::sin(a1 + da * float(j + 1)) * r_outer };
		...
		void CSemiPieChart::RenderBorders(IRenderer *renderer, const PointF &center, const float r_outer, const float r_inner, IChartData *data, const int color_index)
		{
			if (data->getCount() > 1)
			{
				float sum{ 0 };
						...
						if (border_color.A > 0)
						{
							...
							if (is_central)
							{
								...
								for (int j = 0; j < line_count; j++)
									line_points[j] = PointF{ center.X - std::cos(a1 + da * (float)j) * r_outer, center.Y - std::sin(a1 + da * (float)j) * r_outer };
								line_points[line_count] = PointF{ center.X - std::cos(a2) * r_outer, center.Y - std::sin(a2) * r_outer };
