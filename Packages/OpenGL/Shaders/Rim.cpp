		...
		namespace shaders
		{
			namespace rim
			{
				...
				const AnsiString Fragment{
					...
					"uniform vec4 app_RimColor;\n"
					"uniform float app_Shininess;\n"
					"uniform float app_RimPower;\n"
					...
					"	vec4 rim = pow(smoothstep(0.0, 1.0, 1.0 - dot(n, v)), app_RimPower) * app_RimColor;\n"
					"	FragColor = app_AmbientColor + diffuse + specular + rim;\n"
					"	if (app_HasTexture)\n"
