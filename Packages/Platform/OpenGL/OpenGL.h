...
namespace nitisa
{
	...
	class COpenGL final :public virtual INativeGraphics, public CReleasable
	{
	private:
		COpenGL(const bool debug);
	protected:
		void BeforeRelease() override;
	public:
		static COpenGL *Create(IErrorListener *listener);
	...
	private:
		...
		bool m_bCompiled;
	public:
		GLuint const &Id{ m_hID };
		AnsiString const &Error{ m_sError };
		bool const &Compiled{ m_bCompiled };

		COpenGLShader(COpenGL *graphics, const AnsiString &shader_text, const GLenum type);
	...
	public:
		GLuint const &Id{ m_hID };
		AnsiString const &Error{ m_sError };
		bool const &Linked{ m_bLinked };

		inline COpenGLProgram(COpenGL *graphics) :
			m_pGraphics{ graphics },
			m_hID{ graphics->CreateProgram() },
			m_bLinked{ false }
		{

		}

		inline ~COpenGLProgram()
		{
			m_pGraphics->DeleteProgram(m_hID);
		}

		inline void AddShader(const COpenGLShader &shader)
		{
			m_pGraphics->AttachShader(m_hID, shader.Id);
		}
