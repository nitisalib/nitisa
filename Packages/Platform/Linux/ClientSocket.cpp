...
namespace nitisa
{
	namespace platform
	{
		...
		#pragma region Constructor & destructor
			CClientSocket::CClientSocket(PlatformHandle handle, const AnsiString &local_ip, const AnsiString &remote_ip, const int local_port, const int remote_port) :
		...
		#pragma region Getters
			...
			{
				u_long available;
				if (ioctl(*reinterpret_cast<int*>(&m_hHandle), FIONREAD, &available) != 0)
					return 0;
		...
		#pragma region Methods
			...
			bool CClientSocket::Send(const char *data, const int size, const bool inverse)
			{
				if (data && size > 0)
				{
					...
					else
					{
						...
						while (sent < size)
						{
							ret = (int)send(*reinterpret_cast<int*>(&m_hHandle), &arr[sent], size - sent, 0);
							if (ret < 0)
							{
								if (inverse)
									delete[] arr;
							...
							else if ((float)std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_sLastAction).count() * 0.001f > m_fTimeout)
							{
								if (inverse)
									delete[] arr;
								return false;
			...
			bool CClientSocket::Receive(char *buffer, const int size, const bool inverse)
			{
				if (buffer && size > 0)
				{
					...
					while (received < size)
					{
						ret = (int)recv(*reinterpret_cast<int*>(&m_hHandle), &buffer[received], size - received, 0);
						if (ret < 0)
						{
			...
			bool CClientSocket::Peek(char *buffer, const int size, const bool inverse)
			{
				if (buffer && size > 0)
				{
					...
					while (received < size)
					{
						ret = (int)recv(*reinterpret_cast<int*>(&m_hHandle), &buffer[received], size - received, MSG_PEEK);
						if (ret < 0)
						{
							if (errno != EWOULDBLOCK)
								return false;
			...
			bool CClientSocket::Write(const String &data, const bool inverse)
			{
				...
				if (!Send(reinterpret_cast<const char*>(&length), sizeof(length), inverse))
					return false;
				if (length > 0)
					return Send(reinterpret_cast<const char*>(data.c_str()), (int)sizeof(wchar_t) * (int)data.length(), inverse);
				return true;
			}
			...
			bool CClientSocket::Write(const AnsiString &data, const bool inverse)
			{
				...
				if (!Send(reinterpret_cast<const char*>(&length), sizeof(length), inverse))
					return false;
				if (length > 0)
					return Send(reinterpret_cast<const char*>(data.c_str()), (int)sizeof(char) * (int)data.length(), inverse);
				return true;
			}
			...
			ISocket::DataExist CClientSocket::isDataExist(ErrorType &error)
			{
				fd_set check;
				int ret;
				timeval time{ 0, 100 };
				FD_ZERO(&check);
				FD_SET(*reinterpret_cast<int*>(&m_hHandle), &check);
				ret = select(0, &check, nullptr, nullptr, &time);
				if (ret < 0)
				{
					// Error happened
					error = CServerSocket::DecodeError(errno);
					return DataExist::Error;
				}
				if (ret == 1 && FD_ISSET(*reinterpret_cast<int*>(&m_hHandle), &check)) // Socket is ready
				{
					// Check whether data exists
					char buf;
					ret = (int)recv(*reinterpret_cast<int*>(&m_hHandle), &buf, 1, MSG_PEEK);
