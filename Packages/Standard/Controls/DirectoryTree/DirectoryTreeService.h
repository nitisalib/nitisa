...
namespace nitisa
{
	...
	namespace standard
	{
		...
		class CDirectoryTreeService :public virtual ITimerListener, public CControlService
		{
		private:
			CDirectoryTree * m_pDirectoryTree;
			PlatformHandle m_hTimer; // Scroll timer in down state

			void CancelDown(const bool check_capture);
			PointF FormToHScroll(const PointF &pos);
			PointF FormToVScroll(const PointF &pos);
			bool FindActiveItem(IListItem *parent, PointF &current, PointF &item_size, const float line_height);
			void ScrollToActiveItem();
			IListItem *FindActiveItem(IListItem *parent);
			bool ProcessKeyUp();
			bool ProcessKeyDown();
			bool ProcessKeyLeft();
			bool ProcessKeyRight();
			bool ProcessKeyHome();
			bool ProcessKeyEnd();
		public:
			...
			CDirectoryTreeService(CDirectoryTree *directory_tree);
		};
	}
}