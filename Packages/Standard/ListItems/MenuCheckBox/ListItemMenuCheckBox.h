...
namespace nitisa
{
	...
	namespace standard
	{
		class CListItemMenuCheckBoxService;

		class CListItemMenuCheckBox :public virtual IListItemCheckBox, public CListItemMenu
		{
			friend CListItemMenuCheckBoxService;
		public:
		...
		private:
			...
			{
			private:
				CListItemMenuCheckBox *m_pControl;
			public:
				void NotifyOnChange() override;

				CGradientListener(CListItemMenuCheckBox *control);
			};
		private:
		...
		public:
			void(*OnChange)(IListItem *sender); // Event called when checked status changed

			CListItemMenuCheckBox();
			CListItemMenuCheckBox(const String &caption);
			~CListItemMenuCheckBox() override;
