...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CSteppedLineChart::CSteppedLineChart() :
			CCustomCartesianChart(L"SteppedLineChart", true, true, false, false, false, false),
			...
			m_aChartInterpolationPositions{ LineInterpolationPosition::Middle },
			m_pRenderer{ new CSteppedLineChartRenderer() }
		{
			setSize(PointF{ 370, 210 });
		}

		CSteppedLineChart::CSteppedLineChart(IForm *parent) : CSteppedLineChart()
		{
			setForm(parent);
		}

		CSteppedLineChart::CSteppedLineChart(IControl *parent) : CSteppedLineChart()
		{
			setParent(parent);
		}
	...
	#pragma region IControl setters
		bool CSteppedLineChart::setDPI(const Point &value)
		{
			Point old{ getDPI() };
	...
	#pragma region Getters
		const BoolArray &CSteppedLineChart::getChartDrawPoints() const
		{
			return m_aChartDrawPoints;
		}

		const BoolArray &CSteppedLineChart::getChartDrawLines() const
		{
			return m_aChartDrawLines;
		}

		const BoolArray &CSteppedLineChart::getChartDrawBackgrounds() const
		{
			return m_aChartDrawBackgrounds;
		}

		const FloatArray &CSteppedLineChart::getChartNormalPointSizes() const
		{
			return m_aChartNormalPointSizes;
		}

		const FloatArray &CSteppedLineChart::getChartActivePointSizes() const
		{
			return m_aChartActivePointSizes;
		}

		const FloatArray &CSteppedLineChart::getChartNormalPointHoles() const
		{
			return m_aChartNormalPointHoles;
		}

		const FloatArray &CSteppedLineChart::getChartActivePointHoles() const
		{
			return m_aChartActivePointHoles;
		}

		const ColorArray &CSteppedLineChart::getChartNormalPointColors() const
		{
			return m_aChartNormalPointColors;
		}

		const ColorArray &CSteppedLineChart::getChartActivePointColors() const
		{
			return m_aChartActivePointColors;
		}

		const ColorArray &CSteppedLineChart::getChartNormalPointHoleColors() const
		{
			return m_aChartNormalPointHoleColors;
		}

		const ColorArray &CSteppedLineChart::getChartActivePointHoleColors() const
		{
			return m_aChartActivePointHoleColors;
		}

		const FloatArray &CSteppedLineChart::getChartLineWidthes() const
		{
			return m_aChartLineWidthes;
		}

		const ColorArray &CSteppedLineChart::getChartLineColors() const
		{
			return m_aChartLineColors;
		}

		const ColorArray &CSteppedLineChart::getChartBackgroundColors() const
		{
			return m_aChartBackgroundColors;
		}

		const BoolArray &CSteppedLineChart::getChartVerticals() const
		{
			return m_aChartVerticals;
		}

		const std::vector<LineInterpolationPosition> &CSteppedLineChart::getChartInterpolationPositions() const
		{
			return m_aChartInterpolationPositions;
		}
	#pragma endregion

	#pragma region Setters
		bool CSteppedLineChart::setChartDrawPoints(const BoolArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartDrawLines(const BoolArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartDrawBackgrounds(const BoolArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartNormalPointSizes(const FloatArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartActivePointSizes(const FloatArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartNormalPointHoles(const FloatArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartActivePointHoles(const FloatArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartNormalPointColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartActivePointColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartNormalPointHoleColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartActivePointHoleColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartLineWidthes(const FloatArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartLineColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartBackgroundColors(const ColorArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartVerticals(const BoolArray &values)
		{
			if (values.size() > 0)
			{
		...
		bool CSteppedLineChart::setChartInterpolationPositions(const std::vector<LineInterpolationPosition> &values)
		{
			if (values.size() > 0)
			{
				m_aChartInterpolationPositions = values;
				if (m_pCanvas)
					m_pCanvas->setValid(false);
				Repaint(false);
				return true;
	...
	#pragma region Helpers
		void CSteppedLineChart::UpdateFromStyle(IStyle *style)
		{
	...
	#pragma region Paint
		void CSteppedLineChart::RenderData(IRenderer *renderer, const RectF &client, IChartData *data, const int index, ITexture *target, const PointF &xrange, const PointF &yrange)
		{
			if (data && data->getCount() > 0)
			{
				...
				else
				{
					...
					m_pRenderer->setPointSize(CSteppedLineChartRenderer::PointState::Normal, m_aChartNormalPointSizes[index % (int)m_aChartNormalPointSizes.size()]);
					m_pRenderer->setPointSize(CSteppedLineChartRenderer::PointState::Active, m_aChartActivePointSizes[index % (int)m_aChartActivePointSizes.size()]);
					m_pRenderer->setPointHole(CSteppedLineChartRenderer::PointState::Normal, m_aChartNormalPointHoles[index % (int)m_aChartNormalPointHoles.size()]);
					m_pRenderer->setPointHole(CSteppedLineChartRenderer::PointState::Active, m_aChartActivePointHoles[index % (int)m_aChartActivePointHoles.size()]);
					m_pRenderer->setPointColor(CSteppedLineChartRenderer::PointState::Normal, m_aChartNormalPointColors[index % (int)m_aChartNormalPointColors.size()]);
					m_pRenderer->setPointColor(CSteppedLineChartRenderer::PointState::Active, m_aChartActivePointColors[index % (int)m_aChartActivePointColors.size()]);
					m_pRenderer->setPointHoleColor(CSteppedLineChartRenderer::PointState::Normal, m_aChartNormalPointHoleColors[index % (int)m_aChartNormalPointHoleColors.size()]);
					m_pRenderer->setPointHoleColor(CSteppedLineChartRenderer::PointState::Active, m_aChartActivePointHoleColors[index % (int)m_aChartActivePointHoleColors.size()]);
					...
					m_pRenderer->setInterpolationPosition(m_aChartInterpolationPositions[index % (int)m_aChartInterpolationPositions.size()]);
