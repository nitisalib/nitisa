		...
		#pragma region Methods
			...
			bool CClientSocket::Send(const char *data, const int size, const bool inverse)
			{
				if (data && size > 0)
				{
					...
					else
					{
						...
						while (sent < size)
						{
							ret = send(*reinterpret_cast<int*>(&m_hHandle), &arr[sent], size - sent, 0);
							if (ret < 0)
							{
								if (inverse)
									delete[] arr;
							...
							else if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_sLastAction).count() * 0.001 > m_fTimeout)
							{
								if (inverse)
									delete[] arr;
								return false;
			...
			bool CClientSocket::Receive(char *buffer, const int size, const bool inverse)
			{
				if (buffer && size > 0)
				{
					...
					while (received < size)
					{
						ret = recv(*reinterpret_cast<int*>(&m_hHandle), &buffer[received], size - received, 0);
						if (ret < 0)
						{
			...
			bool CClientSocket::Peek(char *buffer, const int size, const bool inverse)
			{
				if (buffer && size > 0)
				{
					...
					while (received < size)
					{
						ret = recv(*reinterpret_cast<int*>(&m_hHandle), &buffer[received], size - received, MSG_PEEK);
						if (ret < 0)
						{
							if (errno != EWOULDBLOCK)
								return false;
			...
			ISocket::DataExist CClientSocket::isDataExist(ErrorType &error)
			{
				...
				if (ret == 1 && FD_ISSET(*reinterpret_cast<int*>(&m_hHandle), &check)) // Socket is ready
				{
					...
					char buf;
					ret = recv(*reinterpret_cast<int*>(&m_hHandle), &buf, 1, MSG_PEEK);
