	...
	namespace script
	{
		...
		namespace cpp
		{
			...
			private:
				IReader *m_pReader;
				ILexic *m_pLexic;
				const Operator *m_pSingleQuote;
				const Operator *m_pDoubleQuote;
				...
				bool ParseSingleQuote(size_t &index, Token &token, const size_t prefix_length);
				bool ParseDoubleQuote(size_t &index, Token &token, const size_t prefix_length, const bool raw);
			public:
				// IParser methods
