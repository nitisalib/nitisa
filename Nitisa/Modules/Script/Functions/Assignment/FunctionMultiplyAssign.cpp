...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionMultiplyAssign::CFunctionMultiplyAssign(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2, IFunction *modify) :
		...
		void CFunctionMultiplyAssign::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionMultiplyAssign::Run()
		{
			if (!m_pOperand1)
			{
				if (m_pErrorListener)
			...
			if (!m_pOperand2->Run())
				return false;
			if (m_pOperand1->Result->type() == VariableType::Float || m_pOperand2->Result->type() == VariableType::Float)
