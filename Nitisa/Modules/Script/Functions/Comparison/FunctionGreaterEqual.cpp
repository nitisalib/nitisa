...
namespace nitisa
{
	...
	#pragma region Constructor & destructor
		CFunctionGreaterEqual::CFunctionGreaterEqual(IErrorListener *error_listener, IFunction *operand1, IFunction *operand2) :
		...
		void CFunctionGreaterEqual::BeforeRelease()
		{
			if (m_pOperand1)
				m_pOperand1->Release();
			if (m_pOperand2)
	...
	#pragma region IFunction methods
		bool CFunctionGreaterEqual::Run()
		{
			if (!m_pOperand1)
			{
				if (m_pErrorListener)
